...
function ManagerChecklistsContent() {
  ...
  const fetchData = async () => {
    try {
      ...
      let checkUrl = `${url}/rest/v1/manager_checklists?select=*,stores(name,code),users!user_id(full_name)&order=checklist_date.desc,created_at.desc&limit=100`
      ...
      let formattedData = Array.isArray(checkData) ? checkData.map(item => ({
        ...
        manager_name: item.users?.full_name || item.created_by
      })) : []
      ...
      const allCheckUrl = `${url}/rest/v1/manager_checklists?select=estatus_supervisor,estatus_admin`
