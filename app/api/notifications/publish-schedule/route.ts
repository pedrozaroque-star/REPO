...
export async function POST(req: Request) {
    try {
        ...
        const notificationPromises = employees.map(async (emp: any) => {
            ...
            const shiftRows = empShifts.map((s: any) => {
                ...
                const dayName = date.toLocaleDateString('es-US', { weekday: 'long' })
                const dayNum = date.getDate()
                const month = date.toLocaleDateString('es-US', { month: 'short' })
                const start = date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
                const end = new Date(s.end_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
