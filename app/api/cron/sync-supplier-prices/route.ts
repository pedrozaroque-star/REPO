  ...
  try {
    ...
    for (const parsed of scrapeResult.items) {
      ...
      if (!mapping || !masterItem || currentCasePrice <= 0) {
        ...
        totalIncreases++
        const annualVol = ESTIMATED_ANNUAL_VOLUMES[parsed.supplierSku] || 200
    ...
    // 5. Guardar historial si hubo variaciones
    if (historyInserts.length > 0) {
      await supabase.from('supplier_price_history').insert(historyInserts)
      console.log(`[Cron:SyncSupplierPrices] ✅ ${historyInserts.length} variaciones de precio registradas en el historial.`)
    }
