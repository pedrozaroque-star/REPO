...
4.  **INVENTARIO (Inventory Group)**:
    ...
    *   **Preparador (Prep Tool)**: \`/inventory/preparador\`. Access: Admin, Manager, Supervisor, Assistant (asistente). Purpose: Production projections, grill cooking pace, and raw meat historical consumption curves. Features: (1) Peak Time Period Cards (Tramos de Tiempo por Horas Pico, e.g. HORA PICO AM 11:00 AM - 2:00 PM) displaying average Lbs/Hour and Total Period Lbs required on the grill. (2) Live 3-minute polling interval with \`cache: 'no-store'\` headers and timestamp cache-busters to keep all devices uniformly synchronized with live Toast sales. (3) 3-month rolling DOW historical baseline for meat projections (\`get_meat_history_avg\`). (4) Dual tablet support (Line Cook & Back Kitchen / Bodega). (5) Haptic vibration alerts before period ends and screen Wake Lock. (6) Waste & Consumption Dashboard with interactive date-picker and bilingual Info (\`i\`) explanations.
