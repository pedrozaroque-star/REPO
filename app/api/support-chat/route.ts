...
25. **COMPRAS E INVENTARIO VIELE & SONS (/admin/compras/viele)**: Módulo integral de compras, conteo y pedidos semanales conectado al portal ClearNine / Sage 100 de Viele & Sons (\`shop.vieleandsons.com\`).
    * **REGLA DE FACTURACIÓN DUAL CONSECUTIVA (Sodas vs Insumos Generales)**: Viele & Sons emite FACTURAS SEPARADAS para sodas (jarabes Bag-in-Box 5 galones: BCLCO, BDICO, BMMLE, BMMOR, BSPRI, BRATE, BSTRA, BZECO, BDRPE) vs los demás insumos (vasos, bolsas Seal2Go, servilletas, cubiertos, químicos) debido a regulaciones fiscales y cuentas de proveedor. Al hacer checkout, si el pedido contiene ambos tipos, el sistema ejecuta AUTOMÁTICAMENTE 2 órdenes consecutivas e independientes en Viele & Sons (Orden 1: Sodas -> Wxxxxxx, Orden 2: Insumos -> Wxxxxxx correlativa), enlazándolas en \`viele_orders\` con \`order_category\` y \`linked_order_number\`. En la interfaz y el historial se visualizan ambas facturas y su total consolidado.
    ...
    * **Comprador Oficial Registrado ("AFV")**: En el sistema Sage 100 y portal de Viele & Sons, el nombre de comprador y CustomerPONo siempre se ha registrado históricamente bajo el código oficial **"AFV"** (con soporte para códigos históricos alternativos como LEWIS, MARK, PV en pedidos especiales).
    ...
    * **Historial y Auditoría (/admin/compras/viele/historial)**: Bitácora completa de órdenes emitidas por tienda con desglose de artículos, cajas, precios unitarios e importes totales, filtro por tipo de factura (Sodas vs Insumos) y enlaces a facturas gemelas.
