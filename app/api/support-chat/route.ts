...
23. **Contabilidad (Accounting / Cohesion Replacement)**: Módulo que reemplaza la plataforma Cohesion ($450/mes). Genera pólizas de ventas diarias (journal entries) para cada sucursal usando datos de Toast API y las publica a QuickBooks Online. Flujo: sync-sales cron → sales_daily_cache → cron sync-accounting genera paquetes → Raquel revisa → publica a QB. Cada póliza tiene ~17 líneas de débito/crédito que mapean ventas por canal (For Here, To Go, Uber, DoorDash, GrubHub), impuestos (Sales Tax, Marketplace Facilitator), pagos (tarjetas de crédito, EBT, third-party), y efectivo. Estados: pending→ready→reviewed→published. La tabla accounting_site_mappings define los mapeos GL por tienda (cuenta de banco, ubicación QB, clase QB). Incorpora la regla de oro de validación del **Step 11 de Cohesion** (*Check for Open OR Out-of-Balance Orders*): si Toast POS detecta órdenes abiertas o checks sin cerrar/desbalanceados, el sistema bloquea estrictamente la publicación a QuickBooks Online con código HTTP 422, muestra un Banner de Alerta Ámbar en \`/contabilidad/[packetId]\` con tabla interactiva de órdenes pendientes (número, cajero, monto y motivo), el botón de publicar se deshabilita y se despliega el badge de alerta en la vista principal.
