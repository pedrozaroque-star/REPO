...
22. **AUDITORÍA LABORAL RONOS & FACTURACIÓN CINGULAR HR** (\`/admin/ronos\`): Módulo corporativo de auditoría de nómina, ponchadas y cumplimiento de leyes laborales de California (IWC Wage Order 5 / Labor Code § 512) conectado directamente a la API REST v2.0 de RONOS y sincronizado permanentemente en Supabase (\`ronos_work_weeks\`, \`ronos_employee_timecards_cache\`, \`ronos_transfers_history\`, \`ronos_employee_mappings\`). Cuenta con 4 pestañas operativas: 1) **Auditoría por Sucursal**: Tarjetas de tiempo, horas regulares, horas extras (OT 1.5x, DT 2.0x), analizador de Meal Penalties (> 4h 59m o < 30m) con exención legal <= 6.0h, tarjetas rotas y cálculo de fuga financiera ($ USD), 2) **Reloj en Vivo**: Monitoreo en tiempo real de personal activo en tienda, 3) **Mapeo de Colaboradores**: Vinculación Toast-RONOS, detección de traslados multi-tienda y envío de avisos oficiales de incumplimiento por correo a la escalera de mando, y 4) **Facturación Cingular & Nómina (Motor de Reconciliación PEO)**: Conciliación matemática al centavo contra facturas oficiales de Cingular HR (ej. invoice-TEGW-0009.pdf). Diferencia personal Asalariado/Exempt (General Manager / Supervisor de Zona con salario fijo 80h bisemanal y markup ~24.51%) vs Por Hora/Non-Exempt (Asistente hacia abajo con 25.98% markup sobre tarifa base: BILL_RATE = PAY_RATE * 1.25976). Extrae automáticamente horas de Enfermedad (Sick Pay), Vacaciones (PTO Vacation) y Feriados (Holiday) capturados en las tabletas RONOS en tienda (workDays vacationHours y sickHours), totalizando salarios brutos (TOT PAY), margen Cingular (Cingular Fee) y facturación total (TOT BILL) con exportación a CSV idéntico al Summary Report oficial. Origen de datos: \`ronos_employee_timecards_cache\` + \`toast_employees.wage_data\`. Omitir placeholder del sistema 'Manager Default'.
