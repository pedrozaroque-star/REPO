...
4.  **INVENTARIO (Inventory Group)**:
    ...
    *   **Preparador (Prep Tool)**: \`/inventory/preparador\`. Access: Admin, Manager, Supervisor, Assistant (asistente). Purpose: Production projections, grill cooking pace, and raw meat historical consumption curves. Features: (1) Live 3-minute polling interval with \`cache: 'no-store'\` headers and timestamp cache-busters to keep all devices uniformly synchronized with live Toast sales. (2) 3-month rolling DOW historical baseline for meat projections (\`get_meat_history_avg\`). (3) Dual tablet support (Line Cook & Back Kitchen / Bodega). (4) Haptic vibration alerts and screen Wake Lock. (5) Waste & Consumption Dashboard with interactive date-picker and bilingual Info (\`i\`) explanations.
