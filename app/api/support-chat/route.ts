...
13. **MILESIQ**: Módulo de control y cálculo de millas manejadas por supervisores. Tarifa oficial de reembolso IRS: $0.760/mi + peajes/parking. Permite registrar origen/destino entre sucursales de Tacos Gavilan con autodetección GPS (1 toque para detectar tienda más cercana en salida o llegada), ida y vuelta (x2), conexión directa e inicio de navegación con Google Maps, Apple Maps y Waze para esquivar tráfico en tiempo real (auto-guarda el viaje antes de abrir la app de mapas), modelo de cálculo con factor de desvío de tráfico de Los Ángeles (1.33x), edición y corrección de viajes pendientes por supervisores, bitácora histórica de envíos y despacho de resúmenes consolidados de nómina directamente a RRHH vía correo electrónico utilizando la cuenta activa del usuario en sesión. Mantiene catálogo de correos recurrentes de RRHH. AUTO-SYNC FROM INSPECTIONS: Cuando un supervisor completa una inspección de calidad en una tienda, el sistema genera automáticamente un trip en MilesIQ con la tienda inspeccionada como destino, asignando automáticamente la hora actual de California y la fecha de negocio correcta (regla 6AM). FILTERED SUBTOTAL: La tabla DriveLog y las tarjetas móviles muestran una fila de subtotal filtrado que suma solo los viajes visibles según los filtros activos de fecha/estado.
...
3.  **INVENTARIO Y MERCANCÍA (Inventory & Merchandise Group)**:
    *   **Radar de Precios (Supplier Price Radar)**: \`/admin/precios-proveedores\`. Access: Admin, Manager, Supervisor. Purpose: Smart clipboard supplier price intake, inflation detection, 15-store COGS impact calculator, and recipe cascade approval. Tooltip: Smart supplier price intake, inflation detection, and 15-store COGS audit.
...
5.  **ADMINISTRACIÓN Y SISTEMA (System & Admin Group)**:
    *   **Tiendas (Stores)**: \`/tiendas\`. Access: Admin. Purpose: Configure active restaurant store metadata and external IDs. Tooltip: Master setup and location data for all Tacos El Gavilan stores.
...
- **Price Increase Radar & Universal Supplier Cost Auditor**: In development to track and audit prices for all 87 packaging/disposable items (Viele & Sons) and future suppliers (Sysco, US Foods). Analyzes uploaded price lists/invoices (CSV/Excel/PDF), flags price increases in red, estimates annual bottom-line impact, and updates recipe unit costs across all 15 stores with 1-click approval.
