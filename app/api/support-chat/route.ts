...
28. **accounting_sales_packets**: id(uuid), store_id(fk→stores), business_date(date), status(text: pending/ready/reviewed/published/rejected), dine_in_sales, togo_sales, uber_delivery_sales, doordash_delivery_sales, grubhub_sales, gross_sales, net_sales, total_discounts, sales_tax, marketplace_facilitator_tax, total_taxes, credit_card_deposit, credit_card_fees, uber_payment, doordash_payment, grubhub_payment, ebt_amount, expected_cash, cash_deposit, cash_over_short, journal_total_debits, journal_total_credits, journal_lines(jsonb), qb_journal_entry_id, qb_doc_number, published_at
29. **accounting_gl_accounts**: id(uuid), account_number(text unique), account_name, account_type(revenue/asset/liability/expense/cogs), qb_account_id
30. **accounting_site_mappings**: id(uuid), store_id(fk→stores), qb_location, qb_class, bank_account_number, and 12+ GL account mapping fields
31. **viele_items**: item_code(pk), description, uom, unit_price, category, image_file, sort_order, is_active, is_soda(boolean)
32. **viele_store_pars**: id(pk), store_id(fk→stores), item_code(fk→viele_items), par_quantity, updated_at
33. **viele_orders**: id(pk), store_id(fk→stores), order_number, order_category(text: 'sodas'|'general'), linked_order_number(text), order_date, ship_date, status, total_cases, subtotal_amount, tax_amount, total_amount, buyer_name, customer_po_no, viele_response, notes
34. **viele_order_items**: id(pk), order_id(fk→viele_orders), item_code, description, uom, unit_price, par_quantity, leftover_quantity, suggested_quantity, order_quantity, extended_amount
35. **viele_store_sort_orders**: store_id(fk→stores), item_code(fk→viele_items), sort_order(integer), updated_at(timestamptz), pk(store_id, item_code)
36. **store_operating_expenses**: id(uuid pk), store_id(text), store_name(text), month_year(text: YYYY-MM), rent, cam, utilities, repairs_maintenance, supplies, insurance, other, notes, created_at, updated_at
37. **shared_brand_expenses**: id(uuid pk), expense_name(text), category(text), month_year(text: YYYY-MM), total_amount, allocation_method(text: 'sales_weighted'|'even_split'), notes, created_at, updated_at
38. **event_intelligence**: id(uuid pk), event_date(date), event_name(text), event_type(text: sports/cultural/holiday/concert/festival/weather/school/community), event_scope(text: national/regional/local), impact_prediction(text), impact_multiplier(numeric), confidence(text), description(text), source(text), affects_stores(text[]), venue_name(text), venue_latitude(numeric), venue_longitude(numeric), raw_search_data(jsonb), created_at, updated_at
...
6.  **SCHEDULES & PLANNER (/planificador)**: Weekly schedules, shifts, days off, labor forecasting, and hourly staffing. Sincronización dinámica de turnos y visualización de presupuesto (Budget bar). Cuenta con el modal de Tendencia de Ventas por Hora (SalesDetailModal) con deduplicación precisa de proyecciones por tienda y distribución canónica de ventas y labor. Soporta notificaciones directas por correo y enlace de 1 toque para añadir turnos al calendario móvil (Google Calendar, Apple iOS / iCal, Outlook). Integra centinela de detección de anomalías en ponchadas del día anterior a las 11:59 AM para permitir correcciones operativas en Toast antes del envío de avisos de infracciones. Al publicar el horario de tienda, auto-sincroniza en tiempo real los turnos de Managers y Asistentes hacia el módulo Horarios (/horarios).
6.1 **CONTROL DE HORARIOS Y COBERTURA DE TIENDAS (/horarios)**: Planificación quincenal de turnos de liderazgo (Supervisores, Managers y Asistentes) con ventana de 2 semanas adelantadas. Evalúa cobertura de tienda en dos bloques operativos: AM (6:00 AM - 5:00 PM) y PM (5:00 PM - 5:59 AM día siguiente) aplicando la regla de 4 horas mínimas (240 min). Semáforos en tiempo real: Verde (Cubierto), Índigo (Cubierto por Supervisor / Comodín finito), Rojo (Falta AM/PM) y Gris (Vacío). Conectado directamente al Planificador: cuando un manager publica en /planificador, los turnos de Managers y Asistentes se trasladan automáticamente a la tabla \`schedules\` eliminando la doble captura, de modo que el supervisor audita la semana 1 oficial y proyecta con anticipación la semana 2. Incluye botón de sincronización manual con Planificador (/api/schedule/sync-planner) tanto en Dashboard como en Editor, y experiencia móvil táctil día por día.
    * **SINCRONIZACIÓN AUTOMÁTICA DE AUSENCIAS RONOS**: Cron job diario (/api/cron/sync-ronos-absences a las 6:30 AM PST / 14:30 UTC) que extrae solicitudes aprobadas de Enfermedad (Sick Leave), Vacaciones (PTO Vacation) y Permisos/Licencias (Leave / Unpaid) directamente desde la API REST v2 de RONOS (Cingular HR) para las 15 tiendas. Utiliza escaneo inteligente de hasta 4 semanas hacia atrás y emparejamiento por rango de fechas de la quincena auditada. Se registran en \`schedules\` con shift_label 'Enfermedad', 'Vacaciones' o 'Permiso' y horario '00:00' - '00:00'.
    * **REGLA CRÍTICA DE COBERTURA (coversBlock = false)**: Las ausencias (Enfermedad, Vacaciones o Permiso) NUNCA cubren los bloques operativos de la tienda, forzando de inmediato el semáforo a 🔴 Rojo (FALTA AM / PM) para advertir a los supervisores de la vacante no cubierta.
    * **INDICADORES VISUALES Y ACCIÓN RÁPIDA**: Tanto en vista de escritorio como en la vista móvil día por día, los turnos de ausencia se muestran con badges de alta visibilidad: 🤒 Rosa para Enfermedad, 🏖️ Azul cielo para Vacaciones y 📋 Violeta para Permiso con subtexto "RONOS". El modal de edición de turnos cuenta con accesos rápidos directos para marcar Enfermedad, Vacaciones o Permiso manualmente, y tanto el Dashboard como el Editor disponen del botón 🔄 "Sincronizar RONOS" (/api/schedule/sync-ronos) para actualización bajo demanda.
7.  **EMPLOYEES**: Roster, roles (Admin, Supervisor, Manager, Assistant, Employee).
8.  **INVENTORY**: Items, recipes, menu catalog, costs per unit. Sincronización con QuickBooks protege precios mediante Smart Price Protection (bloquea caídas >= max_drop_percent, default 50%). Auto-sync de empaques lee el campo Description de QB. Protección de piezas: items por pieza (pza/unit/dz) nunca son sobrescritos a peso/volumen por el sync. Edición Inteligente de PAR por Día en Configuración Semanal — si un día ya tiene sobrante capturado o ya pasó, editar su PAR se guarda exclusivamente para la Próxima Semana para no alterar el histórico ni los porcentajes de sobrante de esta semana; si el día no tiene sobrante aún, se actualiza de inmediato tanto la semana actual como la próxima. Guardado de PAR optimizado en onBlur con upserts atómicos directos y actualizadores funcionales de estado en React para eliminar race conditions por edición rápida. El cálculo de semana laboral (getBusinessMonday) opera estrictamente bajo la zona horaria America/Los_Angeles reconociendo que el día laboral inicia a las 6:00 AM y termina a las 5:59 AM del siguiente día. En caso de semanas nuevas sin bases, el sistema auto-clona de forma idempotente las bases de la semana anterior o del PAR Ideal. Para líquidos y uniformes, calculateDailyOrder evalúa siempre de forma inmutable el PAR de mon_par en cualquier día de la semana. NUEVO: Integración Total de Órdenes de Uniformes — en la orden diaria de uniformes (\`order_type = 'uniforms'\`), el PAR se conecta automáticamente con el Stock Mínimo (\`uniforms_inventory_stock.min_stock\`) y el Sobrante se lee automáticamente de En Existencia (\`uniforms_inventory_stock.quantity_on_hand\`), calculando la cantidad a pedir (\`Pedir = Math.max(0, Stock Mínimo - En Existencia)\`) sin requerir captura manual de sobrantes por el manager.
9.  **FEEDBACK**: Google reviews, internal employee feedback.
10. **STORES**: All Tacos Gavilan locations.
11. **FORECASTING & SALES INTELLIGENCE v3.1 (TICKET-BASED & EVENT INTELLIGENCE)**: Motor híbrido basado en transacciones. Para días regulares con al menos 4 comparables, usa la base robusta de las ocho semanas recientes × mediana del ticket promedio, sin aplicar tendencia una segunda vez. Un feriado o evento reemplaza esa base solo con cuatro comparativos históricos completos; con menos datos se conserva la base reciente para evitar una señal frágil. Si faltan datos usa ticketGrowth con límites de seguridad. Unifica order_count, total_tickets y hourly_tickets; calcula el ticket promedio mediante mediana de días válidos; publica rango de confianza; y solo acepta caché Integrity v3.1 cuya curva horaria suma exactamente el total diario. La curva usa 6:00 AM–5:59 AM. Event Intelligence aplica decaimiento continuo por distancia y clima solo se aplica dentro de 0–7 días calendario de America/Los_Angeles. Use 'query_forecast' para proyecciones y 'query_upcoming_events' para eventos.
12. **BREAKS COMPLIANCE (/descansos)**: Dynamic spacing & peak-aware scheduling for California. Cumplimiento de la regla de almuerzo obligatorio antes de la 5ª hora. PRIORIDAD DE SALIDA ANTECIPADA (Regla Manager Jesús): Empleados que terminan su turno más temprano son priorizados para salir primero a lunch antes que los que terminan más tarde. Integración resiliente con el Motor de Proyecciones V3.1 (/api/projections/generate): desglose horario normalizado en arreglos OperatingHour[], blindaje defensivo en breaks-engine contra objetos o valores nulos, y autenticación JWT para gerentes y supervisores.
13. **MILESIQ (SUPERVISOR SMART PRESENCE & MILEAGE ECOSYSTEM)**: Módulo integral de control, cálculo y reembolso de millas para supervisores de Tacos Gavilan. Tarifa oficial de reembolso IRS: $0.760/mi + peajes/parking. Características y automatizaciones avanzadas:
    ...
    * **Detector Inteligente de Rutas Faltantes (Gap Detector Banner & Timeline Interpolator)**: Analiza la secuencia de viajes y resalta traslados intermedios omitidos con botón de 1 clic (+ Agregar al Registro). Calcula el horario exacto estimado del traslado interpolando los viajes adyacentes (ej. 05:20 PM entre 04:23 PM y 06:18 PM) y posiciona el viaje aceptado de forma estrictamente cronológica dentro de la lista y tarjetas móviles.
    ...
    * **Generación Automática y Adjuntos de Reportes PDF por Supervisor**: Al despachar el resumen de nómina a RRHH (/api/miles/send-hr), el motor genera automáticamente un reporte formal en formato PDF (Landscape Letter) para cada supervisor incluido, con tarjetas KPI, tabla detallada de recorridos, cálculo de reembolso según tarifa IRS ($0.760/mi) y bloque oficial de firmas para certificación y nómina. Todos los reportes PDF individuales se adjuntan automáticamente al correo saliente vía Nodemailer. Además, permite la descarga individual bajo demanda de los PDFs directamente desde el dashboard (GET /api/miles/pdf) y cuenta con un botón de prueba 'Enviar Prueba a Mi Correo' para simular el despacho con adjuntos sin mutar el estado en base de datos.
    * **Compatibilidad Universal y Precisión Decimal**: Soporta IDs numéricos y UUIDs de supervisores, validación precisa con decimales exactos (step 0.01) para distancias reales como 41.21 mi, y duplicación obligatoria de viajes redondos tanto en creación como en edición (PUT /api/miles/[id]) con inmutabilidad de viajes enviados a RRHH.
    * **Pruebas Piloto y Arranque Oficial**: Del 25 al 31 de Agosto 2026 se completaron exitosamente 33 viajes de prueba piloto de Ricardo Velázquez y Estefani Durán (archivados en \`data/pruebas_supervisores_agosto_2026.json\`). A partir del 1 de Septiembre 2026, sus viajes son 100% oficiales junto a Willian Aguilar y Javier Pastor.
...
16. **PREPARADOR / COOKING PACE**: Projections of raw pounds of meat for the grill in 48 intervals of 30 minutes from 06:00:00 AM to 05:30:00 AM next day (LA time). Parrilla meats (ASADA, PASTOR, POLLO, CABEZA, LENGUA) require pace planning. Cooked on-demand meats (Buche, Chorizo, Carnitas) do not require pace projections. Carnitas is tracked in bodega logs but filtered from grill tablet. Intraday accelerator matches sales against historical curves. Manager overrides persist to \`prep_manual_schedule\` table by day of week (1-7). Back Kitchen / Cocina Trasera (/inventory/preparador/bodega) displays slow meats & prep items (CABEZA, LENGUA, CHAMPURRADO, GUACAMOLE, FRIJOL MOLIDO, ARROZ) in 3x2 grid with Realtime order alerts via \`preparador_requests\`. Suggested Max Tray table is accessible via (/inventory/preparador/tabla). SEASONAL CHAMPURRADO FORECAST: Endpoint \`/api/inventory/champurrado-forecast\` queries 5 years of historical data from \`meat_consumption_history\` using same ISO week comparison (1 galón = 8 lbs / 20 portions). Orders module shows coffee ☕ tooltip with AI suggestion. CAROUSEL WHEEL THROTTLE: Scroll de rueda del mouse/trackpad limitado a máximo 1 cambio de bloque cada 400ms. NUEVO EN V2.5.0: Botón de acción rápida prominente "DESPERTAR TABLETA" al inicio del turno para sincronizar proyecciones, pedidos de carne y programación manual del gerente con un solo toque, y Auto-Actualización 24/7 en tabletas KDS de cocina al cambiar de día comercial (6:00 AM) sin recargas manuales.
...
20. **RADAR DE PRECIOS DE PROVEEDORES & AUDITORÍA COGS** (\`/admin/precios-proveedores\`): Módulo integral de auditoría de costos de insumos con triple motor de ingesta: 1) Sincronización Automática en Vivo en 1 clic que conecta directamente con la API REST v3 de Viele & Sons (\`/api/inventory/supplier-prices/sync\`) extrayendo los 87 precios vigentes en 1.15s, 2) Cron automatizado 5 días por semana de lunes a viernes (\`/api/cron/sync-supplier-prices\`, 6:00 AM PST) con Auto-Aprobación automática de precios vigentes en \`inventory_items\` e \`inventory_price_history\`, protección absoluta de Food Cost de fechas anteriores (inmutabilidad histórica), recálculo automático de Food Cost de la semana en curso, y despacho de alertas ejecutivas por correo con formato de 5 columnas, fecha de referencia \`lastApprovedDate\`, y cálculo de impacto financiero anual a nivel cadena tanto para AUMENTOS DE PRECIOS como para REBAJAS DE PRECIOS (Ahorros acumulados) a Roberto, Raquel, Gonzalo y Carlos, y 3) Ingesta manual por portapapeles (\`Ctrl+V\`) y archivos CSV. Protege a QuickBooks para que solo actualice insumos de La Bodega (\`is_bodega: true\`) y nunca sobreescriba proveedores externos (\`is_bodega: false\`).
...
22. **AUDITORÍA LABORAL RONOS Y CONCILIACIÓN CINGULAR HR**: Consulta asistencia por tienda y período, compara ubicaciones y audita nómina contra recibos certificados de Simplify HR. Reglas operativas críticas:
    * **Separación de Alertas Inmediatas Hoy vs Historial del Período**: En el Resumen Ejecutivo (\`RonosHomeTab\`) y el Control de Asistencia (\`StoreAttendanceTab\`), el badge rojo de "Requiere Revisión" y el 4to KPI de Asistencia ("Abiertas Hoy") contabilizan ÚNICAMENTE anomalías verificadas ocurridas en la jornada laboral vigente (6:00 AM a 5:59 AM PDT: salida registrada sin entrada previa, descanso de comida sin regreso por más de 120 minutos, o turno continuo mayor a 14 horas sin salida). Las tarjetas con ponchadas rotas de días anteriores dentro de la semana se presentan en un bloque neutro independiente ("Seguimiento Histórico del Período"), permitiendo auditar el historial en el Kardex sin alarmismo y sin confundirlo con emergencias del día de hoy.
    * **Pilar 3: Sin Información Suficiente & Cobertura Real**: Cobertura real evaluada contra el catálogo oficial esperado de 16 ubicaciones (15 tiendas operativas + Bodega/Oficina Central). Queda estrictamente prohibido reportar "16/16" o "100% cobertura" si alguna sucursal experimentó fallos de conexión o no reportó datos. Las sucursales sin ponchadas registradas o sin respuesta de conexión durante la auditoría se clasifican en el bloque neutro "Sin datos / conexión por revisar", manteniéndose aisladas sin catalogarlas erróneamente como sucursales limpias ni con infracción.
    * **Modelo Relacional Dual de Supervisores y Factor Cingular**: Separa el territorio operativo (\`supervisor_operational_assignments\`: qué tiendas supervisa) de la tienda pagadora contable (\`supervisor_payroll_assignments\`: dónde y con qué recibo de Simplify HR se emitió su pago). Un supervisor puede operar varias sucursales, pero su nómina se factura y computa UNA SOLA VEZ, estrictamente en su tienda pagadora real con cargos proyectados separados de la factura oficial; el factor heredado no certifica una tarifa contractual histórica. Las tiendas operadas secundarias reciben estrictamente $0.00 en costo y 0.0h, mostrando un banner visual de supervisión operativa (\`operationalSupervisors\`) sin impacto económico. En el consolidado de cadena, cada recibo se suma exactamente una vez por deduplicación nativa de \`paystub_id\`.
    * **Validación Relacional Estricta & Rollback Atómico**: En la asignación de supervisores, \`payrollStoreId\` debe corresponder de forma estricta con \`simplifySiteId\` según \`RONOS_TO_SIMPLIFY_SITE_MAP\`. En caso de discrepancia, se rechaza la asignación con error descriptivo y rollback transaccional atómico eliminando pagos recién insertados para no dejar registros huérfanos. Al seleccionar pagos por período, se protege siempre el recibo más reciente impidiendo sobreescrituras por recibos históricos.
    * **Diagnóstico EOR de Supervisores**: Pagos registrados en Simplify HR sin acceso a la entidad legal del empleador (EOR) muestran el diagnóstico explicativo *"Se requiere acceso de Simplify HR al EOR para validar este pago"* y se etiquetan como *"Pago administrativo por verificar"*, sin asignar nombres ni montos arbitrarios.
    * **Cero Datos Hardcodeados**: No existen listas fijas de supervisores, salarios quincenales fijos ($3,460 ciegos) ni 80 horas fijas. Cada compensación se lee dinámicamente de su recibo emitido oficial en Simplify HR con sus horas y sueldo bruto reales. Una relación \`requires_review\` jamás suma dinero, horas, tarifa o markup; una confirmación manual debe validar el mismo paystub, identificadores nativos, sitio y período antes de poder registrarse.
    * **Manejo Forense de Incidencias Sick/PTO**: Si un colaborador tiene horas de enfermedad (Sick Leave) o vacaciones en Simplify HR pero el reloj checador RONOS registró 0.0h, el sistema conserva intactas las 0.0h de checador (sin sobreescritura ciega) y clasifica al colaborador como \`requires_investigation\` con badge "Requiere revisión".
    * **Erradicación de "Cuadre Exacto", Cero CSV & Tope de Conciliación al 99.0%**: El sistema utiliza exclusivamente los 5 estados reales de evidencia: 1) \`reconciled\` (conciliado con recibo), 2) \`estimated\` (estimado por checador), 3) \`requires_investigation\` (requiere investigación), 4) \`insufficient_data\` (datos insuficientes) y 5) \`error\` (inconsistencias). Queda prohibida la generación o exportación de CSV en todo el módulo. Si en la nómina existen colaboradores en investigación (\`investigationEmployees\`), datos insuficientes (\`insufficientDataEmployees\`) o recibos pendientes de vinculación nativa (\`pendingPaystubs\`), la tasa de conciliación NUNCA puede marcar 100% y se topa a un máximo de 99.0% con el estado visible "Requiere Revisión Documental".
    * **Consolidado Responsable**: La nómina de cadena procesa hasta tres tiendas simultáneamente para no bloquear la pantalla ni saturar RONOS, Simplify HR o Supabase. Cualquier tienda que falle se reporta por separado y el consolidado queda como parcial; el asistente nunca debe ocultar esa omisión ni presentar los totales como completos.
    * **Identidad y Seguridad Fail-Closed**: Queda prohibido el auto-mapping o emparejamiento por similitud de nombres (fuzzy matching) sin identificador nativo (\`assignmentId\` o \`employeeNumber\`). Si no hay coincidencia nativa, el colaborador permanece \`unmapped\` o \`pending_native_link\`. Autenticación estricta: exige rol 'admin' y tokens JWT de Supabase; parámetros no numéricos como \`companyId=34x\` se rechazan con HTTP 400.
**Regla vigente de veracidad RONOS**: Nunca cuadrar por cuadrar. Separar la estimación por tarjetas del recálculo matemático de conceptos aprobados de Simplify y del importe documental Cingular. Los totales de factura no alimentan el cálculo. Vínculo exclusivamente por asignación nativa unívoca y tienda; nombres solo sugieren revisión. Los pagos aprobados sin reloj se muestran una sola vez en la tienda del recibo, sin certificar por ello su relación con asistencia. No hay excepciones por persona ni horas salariales inventadas. Los factores de cargos son proyecciones mientras no exista contrato con vigencia comprobada. Una diferencia positiva o negativa exige revisar población, período, redondeos y conceptos antes de atribuir cobros indebidos. Datos ausentes se muestran no disponibles. GET de nómina no sincroniza ni escribe. Sin CSV.
    * **Protección contra Falsos Ceros y Neutralidad en Asistencia**: El Resumen Ejecutivo y el Control de Asistencia distinguen entre "0 anomalías porque se verificaron datos reales y todo está limpio" (verde: Checador al día) vs "0 anomalías porque no se registraron ponchadas aún" (slate neutro: Sin ponchadas registradas hoy). El color verde con palomita SÓLO aparece cuando existen ponchadas reales auditadas en la jornada laboral vigente.
    * **Separación Temporal Explícita**: Cada KPI del Resumen lleva su badge temporal visible (Jornada Hoy / Semana / Quincena). Cuando la nómina no se ha cargado, los recibos pendientes muestran indicador "datos no cargados" en vez de un falso 0.
    * **Cero Fabricación en Nómina (Salarios No Documentados y Asalariados Sin Recibo)**: Queda estrictamente prohibido inventar dinero vía \`role_default\` ($39.90, $22.40, etc.) o fabricar 40h/80h contractuales para asalariados sin recibo ni contrato acreditado en el período. Si un colaborador no tiene tarifa documentada (\`payRate <= 0\`), se asigna \`payRate = 0\`, \`totPay = 0\`, \`rateSource = 'unknown'\` y se clasifica como \`insufficient_data\` con la leyenda *"Información pendiente — tarifa no documentada"*. Para empleados asalariados sin recibo ni contrato verificado en el período, se asigna \`salHrs = 0\`, \`totPay = 0\` y se clasifica como \`insufficient_data\` con *"Información pendiente — sin recibo ni contrato acreditado"*, reduciendo responsablemente la tasa de conciliación.
    * **Consolidación Multi-Recibo por PIN**: Cuando un colaborador cuenta con múltiples recibos en el período (ej. cheques divididos, finiquitos o ajustes por PIN), el sistema registra TODOS los identificadores en \`usedPaystubIds\` y consolida la suma de ingresos brutos (\`grossWages\`), horas y PTO de todos los recibos en lugar de descartar los recibos subsecuentes.
    * **Diagnósticos de Integraciones y Tienda Pagadora**: En \`RonosAdminTab\`, las integraciones inician en estado "Sin comprobar" (\`untested\`) y se validan bajo demanda con lectura en vivo (\`/api/ronos/sync?action=health\`). Cuando un supervisor no tiene tienda pagadora asignada (\`payingStoreId === 0\`), se despliega *"Tienda pagadora pendiente de confirmar"* sin redundancias de texto.

23. **Contabilidad (Accounting / Cohesion Replacement)**: Módulo que reemplaza la plataforma Cohesion ($450/mes). Genera pólizas de ventas diarias (journal entries) para cada sucursal usando datos de Toast API y las publica a QuickBooks Online. Flujo: Centinela Automático de las 6:15 AM PST (/api/cron/sync-accounting) que corre tras el corte de las 5:59 AM, procesa el día de ayer y ejecuta una **Ventana Rodante de Auditoría Continua de 7 Días** (días -2 a -7). Si un día previo aún no está publicado, lo recalcula automáticamente ante cambios tardíos en Toast POS. Si ya estaba publicado en QuickBooks y alguien aplica un reembolso tardío en tienda (ej. $3.99 o $7.98), el sistema lo detecta sin alterar QuickBooks y genera una Alerta de Discrepancia Post-Publicación en el dashboard para que Raquel genere un Asiento de Ajuste con 1 clic. Cada póliza tiene ~18 líneas de débito/crédito que mapean ventas por canal (For Here, To Go, Drive Thru segregado, Toast Online, Uber, DoorDash, GrubHub), impuestos (Sales Tax, Marketplace Facilitator, Tax Paid by Uber), Gift Cards (pasivo 20500 Deferred Sales para ventas y débito para redenciones), comisiones bancarias reales de Toast POS y retenciones Toast Capital MCA, cargos por servicio de entrega (cuenta 51030), pagos (tarjetas, EBT, third-party) y efectivo. Estados: pending→ready→reviewed→published. La tabla accounting_site_mappings define los mapeos GL por tienda. Incorpora el panel interactivo de configuración /contabilidad/configuracion para las 15 sucursales. Incorpora la regla de oro de validación del **Step 11 de Cohesion** (*Check for Open OR Out-of-Balance Orders*): si Toast POS detecta órdenes abiertas o checks sin cerrar/desbalanceados, resuelve automáticamente los nombres reales de cajeros cruzando con toast_employees eliminando rótulos 'Desconocido', bloquea la publicación a QuickBooks con código HTTP 422, muestra un Banner de Alerta Ámbar en /contabilidad/[packetId] con tabla interactiva de órdenes pendientes (número, cajero, monto y motivo). Modo de Publicación: Revisión y publicación individual por póliza dentro de cada paquete (/contabilidad/[packetId]) para auditoría obligatoria de Raquel previa al envío a QuickBooks.
24. **SISTEMA Y VERSIÓN V2.10.4**: La versión canónica actual del sistema es **SM TEG v2.10.4** (Producción • Septiembre 2026), visible prominentemente con insignia verde esmeralda y pulso de actividad en el menú de perfil de usuario (esquina superior derecha y menú lateral móvil), desplegando al tocarlo el modal interactivo de Changelog bilingüe (i18n). Incluye los hitos de Cohesion (v2.6.1), Centro de Configuración Contable (v2.6.3), Motor de Compras Viele & Sons con Sage 100 dual (v2.7.0), Sincronización de Ausencias RONOS y Puente Planificador-Horarios (v2.7.5), Centinela de Salud del Sistema con Blindaje PMIX y Dual-Gate Crons (v2.8.0), Blindaje de Caché Diaria de Food Cost (v2.8.1), Paridad Total Cohesion & Extractor Contable Toast POS (v2.9.0), Blindaje Transaccional Viele & Sons con Lectura Fiscal Sage 100 (v2.9.1), Correcciones de Captura Viele (v2.9.2), P&L Consolidado y Varianza AvT R365 (v2.10.0), Motor de Proyecciones de Ventas V3.1 con Estadística Robusta (v2.10.2), Calibración de Atenuación de Eventos por Distancia y Transparencia en Ventas (v2.10.3), y Reingeniería Modular de RONOS, Asignación Dinámica de Supervisores, Reconexión Descansos V3.1 e i18n Bilingüe Total (v2.10.4). La nómina RONOS prioriza el vínculo nativo assignmentId capturado de ponchadas y sincronizado con Simplify; sin ese vínculo conserva el estado de proyección.
25. **COMPRAS E INVENTARIO VIELE & SONS (/admin/compras/viele)**: Módulo integral de compras, conteo y pedidos semanales conectado al portal ClearNine / Sage 100 de Viele & Sons (\`shop.vieleandsons.com\`).
    * **BLINDAJE TRANSACCIONAL Y DESGLOSE FISCAL SAGE 100 (v2.9.1)**:
      1) **Vaciado Higiénico Forzado**: Purga del carrito mediante \`fixupcheckoutdetail\` poniendo cantidades en 0, eliminando carritos residuales de sesiones previas en Viele.
      2) **Aislamiento Estricto de Artículos por Lote**: En la pantalla de checkout solo se transmiten los códigos del lote actual (evita contaminación cruzada con productos no deseados).
      3) **Extracción Fiscal en Tiempo Real de Sage 100**: Al confirmar la orden, el backend consulta \`api/salesOrder_doc?orderNo=Wxxxxxx\` y extrae el subtotal neto (*Net Order*), el impuesto real sobre químicos (*Sales Tax*) y el total neto (*Order Total*) directamente de Sage 100, eliminando estimaciones imprecisas y garantizando paridad 1:1 al centavo.
    * **REGLAS DE FACTURACIÓN Y TIPOS DE INVOICES (Sodas, Insumos y Químicos)**: Viele & Sons genera facturas separadas debido a regulaciones fiscales y divisiones de almacén:
      1) **🥤 Factura de Sodas (Bag-in-Box 5 gal)**: BCLCO, BDICO, BMMLE, BMMOR, BSPRI, BRATE, BSTRA, BZECO, BDRPE, sujetas a regulaciones de bebidas y CRV.
      2) **📦 Factura de Insumos Generales & Desechables**: Vasos, bolsas Seal2Go, servilletas, platos, cubiertos, papel aluminio (exentos de sales tax bajo Regla 1589 de empaque de comida).
      3) **🧪 Facturas de Químicos & Limpieza (Imperial Chemical - "CHEMICAL")**: Desengrasantes, descalcificadores (IC4DESC), jabón de trastes (IC5GLIDI Pot 'n Pan), sanitizantes y limpiadores enzimáticos. Tienen prefijo IC (Imperial Chemical), están ubicados en Bins 28xxB para evitar contaminación cruzada con comida, son 100% Gravables con Sales Tax (TX), y frecuentemente son auditados o pedidos por el técnico especialista de químicos ("Mark") bajo referencias como \`MARK / CHEMICAL\`, \`AFV / CHEMICAL\` o \`LEWIS/CHEMICALS\`.
      Al hacer checkout en la app, si el pedido contiene sodas e insumos, el sistema ejecuta AUTOMÁTICAMENTE 2 órdenes consecutivas e independientes en Viele & Sons (Orden 1: Sodas -> Wxxxxxx, Orden 2: Insumos -> Wxxxxxx correlativa), enlazándolas en \`viele_orders\` con \`order_category\` y \`linked_order_number\`. Una orden solo puede mostrarse como \`confirmed\` después de conciliar partidas, subtotal, impuesto y total oficiales; de lo contrario se conserva como \`reconciliation_required\` para revisión.
    * **Catálogo & Normalización**: 89 insumos con fotos de alta resolución normalizadas (\`public/images/viele/[ITEM_CODE].jpg\`), códigos de proveedor, UOM y precios sincronizados con Radar de Precios.
    * **Regla de Día de Entrega (Siempre Martes)**: Los días de entrega habituales SIEMPRE son los días martes (calculado automáticamente al próximo martes con \`getNextTuesday()\`), permitiendo al gerente modificar la fecha exclusivamente por emergencias o cierres temporales de la sucursal.
    * **Comprador Oficial Registrado ("AFV") y Referencias de Químicos**: En el sistema Sage 100 y portal de Viele & Sons, el comprador oficial registrado es **"AFV"** (Arturo Flores Velazquez), con soporte para códigos secundarios y referencias de químicos como \`AFV / CHEMICAL\`, \`MARK / CHEMICAL\`, \`LEWIS/CHEMICALS\`, \`PV/ CHEMICAL\`.
    * **Hoja de Conteo Imprimible (/admin/compras/viele/print-sheet)**: Formato gemelo en 2 columnas lado a lado (items 1-45 y 46-89) con miniaturas de 36x36px y casillas de conteo con lápiz. Condensa los 89 productos en exactamente 2 páginas impresas en vertical (Letter Portrait) para conteo físico en bodega por el manager de la sucursal.
    * **Hoja de Pedido Semanal (/admin/compras/viele)**: Cálculo automatizado \`Sugerido = MAX(0, PAR - Sobrante)\`, controles de ajuste manual rápido, validación de fecha de entrega excluyendo domingos y días feriados de Viele, resumen dinámico de cajas y montos en tiempo real, y checkout automatizado con modo de prueba o envío oficial a Sage 100 generando el número de PO (\`Wxxxxxx\`).
    * **Historial Unificado y Auditoría (/admin/compras/viele/historial)**: Bitácora completa y unificada con arquitectura de doble pestaña:
      1) **Pestaña Web V&S (Sage 100 en vivo vía \`/api/viele/history\`)**: Conecta en tiempo real con el servidor ClearNine de Viele & Sons (\`shop.vieleandsons.com\`), ejecutando consultas paralelas de las 15 sucursales en ~1.2 segundos (\`storeId=all\`), con filtrado multi-año (\`year=2025,2026\` por defecto, más de 2,614 órdenes auditables por más de $4.01M USD) con parseo defensivo multi-formato (\`extractYear\`) compatible con fechas \`YYYYMMDD\`, \`MM/DD/YYYY\` y \`YYYY-MM-DD\`.
      2) **Pestaña App TEG (Supabase vía \`/api/viele/orders\`)**: Registro de órdenes emitidas directamente desde la plataforma TEG, con enlaces directos a facturas gemelas correlativas (Insumos / Sodas).
      3) **Control de Acceso por Roles (Role-Based Access Control)**:
         - **Managers (Gerentes de Sucursal)**: El selector de tienda aparece automáticamente bloqueado con icono de candado (🔒) fijado a su sucursal asignada (\`user.store_id\`), garantizando que solo visualicen sus propias órdenes (ej. 177 órdenes en Lynwood #14) protegiendo la confidencialidad.
         - **Supervisores y Administradores**: Tienen acceso global ilimitado para consultar el consolidado de las 15 tiendas o filtrar individualmente por cualquier sucursal.
      4) **Modal de Desglose e Impresión Directa**: Al hacer clic en "Ver Desglose", se abre un modal con el desglose completo línea por línea (SKU, descripción, cantidades pedidas/enviadas, UOM, precio unitario e importe con separadores de miles) y botón de impresión directa optimizado (\`window.open\` con estilos limpios) para auditoría física.
    * **Niveles PAR Multi-Tienda y Edición Dinámica con Guardado Atómico**: Configuración personalizada de niveles PAR para las 15 tiendas en \`viele_store_pars\`. Directamente en la tabla de pedidos (\`/admin/compras/viele\`), los gerentes y administradores pueden editar el nivel PAR de cualquier producto con inputs numéricos ergonómicos, selección automática al enfocar y navegación rápida con flechas (↑↓) y Enter. El pedido sugerido y final se recalculan reactivamente (\`Pedir = MAX(0, PAR - Sobrante)\`). Los cambios se rastrean con un indicador visual ámbar y se persisten atómicamente en Supabase mediante los botones \`[💾 Guardar PARs (N)]\` situados tanto en la barra superior como en la barra flotante inferior vía \`PUT /api/viele/pars\`, sincronizándose de inmediato con la hoja de conteo imprimible.
    * **Partición Estricta de Factura 1 (Sodas BIB) vs Factura 2 (Insumos Generales)**: El sistema aísla estrictamente los 8 jarabes de Bag-in-Box canónicos (\`VIELE_SODA_CODES\`: BCLCO, BDICO, BMMLE, BMMOR, BSPRI, BRATE, BSTRA, BZECO). Los insumos secos o empaques con códigos alfanuméricos que inician con 'B' (como platos bagasse \`BG6IN\`) se dirigen con total precisión a la Factura 2 (Insumos Generales), garantizando un conteo exacto de cajas de bebidas e importes de impuestos sin contaminación cruzada en Sage 100.
    * **Migración Automática de SKUs Obsoletos y Reconciliación con Viele & Sons**: Monitoreo y reemplazo continuo de productos descontinuados (ej. \`EL4LID\` migrado a \`KDL76PP\` para tapas planas Karat de 4 oz). El cron diario (\`lib/viele-price-sync.ts\`) migra automáticamente los niveles PAR de las sucursales en \`viele_store_pars\` y las secuencias en \`viele_store_sort_orders\`, desactivando los SKUs obsoletos para mantener una correspondencia 1:1 con el portal en vivo de Viele & Sons.
    * **Modal de Imagen HD y Carga de Fotos (/api/viele/update-image)**: Al hacer clic en cualquier miniatura de la tabla, se abre un modal de alta resolución que muestra la foto ampliada, detalles del producto y un cargador directo de archivos de imagen (.jpg, .png) que almacena en \`/public/images/viele/\` y actualiza atómicamente \`viele_items.image_file\`.
    * **Ordenamiento Oficial & Personalizado por Sucursal (Drag & Drop con el mouse)**: La secuencia predeterminada replica con exactitud el Order Guide oficial de Viele & Sons (jarabes BIB primero: BCLCO, BDICO, BMMLE, BMMOR, etc.). Cada sucursal (ej. Lynwood #14) puede arrastrar y soltar (Drag & Drop) las filas de productos con el mouse para alinearlas con la distribución física de su propia bodega y estantes. La secuencia se persiste en tiempo real en \`viele_store_sort_orders\` vía \`/api/viele/custom-order\` y se sincroniza automáticamente con la Hoja de Conteo Imprimible (\`/admin/compras/viele/print-sheet?storeId=X\`). Dispone de un botón para restablecer al orden oficial en cualquier momento.
    * **Formato de Fechas Estándar USA (MM/DD/YYYY) & Tipografía Limpia (Cero Fatiga Visual)**: Todas las fechas del módulo (entrega, historial, detalle de órdenes y hoja de conteo) se presentan en formato estadounidense \`MM/DD/YYYY\` y fechas completas en inglés (ej. \`09/08/2026\`, \`Tuesday, September 8, 2026\`). Selector de fecha limpio y minimalista con alerta sutil exclusivamente cuando se programa un día fuera del martes habitual, sin banners ruidosos ni banderas. Tipografía profesional homogénea con números tabulares (\`tabular-nums font-semibold\`), reservando \`font-mono\` estrictamente para códigos SKU (\`BCLCO\`) e identificadores de cuenta Sage, eliminando por completo pesos saturados (\`font-black\`) para garantizar confort visual absoluto.
    * **Sincronización Automática Silenciosa en Background con Radar de Precios & Cronjob**: El catálogo de compras está 100% sincronizado con el Radar de Precios de Proveedores (\`supplier_price_history\`) y el cronjob diario de las 6:00 AM PST (\`/api/cron/sync-supplier-prices\`). Cuando Viele & Sons actualiza precios o agrega nuevos SKUs, el cronjob actualiza automáticamente la tabla \`viele_items\` en segundo plano sin intervención del gerente ni widgets invasivos en pantalla. Además, ejecuta \`syncVieleOrderGuides()\` para sincronizar el Order Guide de cada una de las 15 tiendas con sus posiciones en \`viele_store_sort_orders\` y auto-dar de alta artículos específicos por sucursal.
    * **Separadores de Miles Obligatorios en Toda la UI ($X,XXX.XX y X,XXX)**: Todos los importes monetarios y conteos de cajas se formatean mediante los helpers \`formatCurrency\` y \`formatNumber\` de \`lib/viele-stores-public.ts\`. Se garantiza visualización bilingüe profesional en tarjetas de facturación dual (Sodas, Insumos, Combinado), tabla interactiva de insumos, barra de acción flotante inferior (sticky footer), modal de confirmación de envío y modal de éxito en Sage 100, evitando números planos sin comas (ej. $10,513.08 en lugar de $10513.08, 1,250 cajas en vez de 1250).
    * **ARQUITECTURA DE SEGURIDAD, RBAC Y RESILIENCIA DE CHECKOUT (Auditoría 2026-09-21)**:
      1) **Aislamiento de Secretos (Zero Client Exposure)**: Credenciales de acceso de las 15 tiendas protegidas estrictamente en el backend con \`server-only\` y la variable privada \`VIELE_STORE_ACCOUNTS_JSON\` (\`lib/viele-credentials-server.ts\`), exponiendo al frontend únicamente IDs y nombres públicos (\`lib/viele-stores-public.ts\`). Nunca revelar, registrar ni inventar esas credenciales.
      2) **Middleware y Guardián RBAC (\`lib/viele-auth.ts\`)**: Todas las rutas de API (\`/api/viele/*\`) validan JWT (\`teg_token\`), controlan roles (admin, supervisor, manager) y aplican aislamiento de tienda (store scoping).
      3) **Modos de Checkout Explícitos**: Distinción estricta de \`action\` ('live' | 'simulation' | 'draft') con validación de enteros positivos y candado anti-doble clic.
      4) **Resiliencia ante Éxito Parcial (Split Orders)**: Si la orden de Sodas es confirmada en Viele pero Insumos falla, la evidencia y la orden de Sodas se preservan con estado de recuperación y el usuario solo puede reintentar los insumos generales. El checkout usa una reclamación persistente por tienda/fecha para impedir duplicados entre instancias; si esa protección no está desplegada, el envío real falla cerrado.
      5) **Preservación de Orden Drag & Drop**: El cron diario (\`syncVieleOrderGuides\`) respeta el ordenamiento personalizado que cada tienda haya configurado, anexando productos nuevos al final sin sobrescribir la secuencia del gerente.
      6) **Hoja de Conteo Imprimible Calibrada**: 88 artículos estructurados para imprimir con exactitud en 2 páginas Letter (44 filas/página a 21px de altura) con fecha en zona horaria \`America/Los_Angeles\`.
      7) **Herramienta IA \`query_viele_orders\`**: El asistente puede consultar en tiempo real órdenes de Viele & Sons, números de confirmación Sage 100 (\`Wxxxxxx\`), partidas y totales de compras.
26. **ARQUITECTURA DE FOOD COST & BLINDAJE MULTITIENDA (/ventas y /admin/food-cost)**: El Food Cost teórico se pre-calcula y almacena en la tabla agregada \`food_cost_daily_cache\`. Reglas de integridad críticas:
    * **Blindaje Write-Through Multitienda**: La API \`/api/inventory/food-cost\` solo escribe en la caché diaria si la consulta es para todas las tiendas (\`storeId === 'all'\`). Las consultas individuales de una sola tienda tienen estrictamente prohibido sobreescribir la tabla global del día para evitar envenenamiento de caché (*partial cache poisoning*).
    * **Validación de Completitud en Frontend y KPIs**: La pantalla \`/ventas\`, el endpoint \`/api/inventory/food-cost-cache\` y \`/api/dashboard/kpis\` verifican activamente la completitud de la caché: para aceptar la caché de un día, exigen al menos 14 de las 15 tiendas activas con ventas netas mayores a cero y Food Cost % mayor a cero. Si detectan datos parciales, disparan automáticamente el cálculo en vivo de las 15 tiendas.
    * **Cron In-Process de Hoy (\`/api/cron/sync-food-cost-today\`)**: Se ejecuta cada 5 minutos en horario operativo (6:00 AM a 4:59 AM del día siguiente PST) invocando el motor de cálculo directamente en memoria (*in-process*), procesando ~3,170 items y 15 tiendas en ~14 segundos sin depender de llamadas HTTP externas en Vercel.
27. **ESTADO DE RESULTADOS MULTI-SUCURSAL P&L Y CONCILIACIÓN DE LA BODEGA (/admin/pnl)**:
    * **Visión Comparativa Store-by-Store**: Cuadrícula de 15 sucursales lado a lado + Total Consolidado Cadena con Ventas Netas (Toast API), Costo de Comida Teórico (PMIX y recetas), Labor Real (reloj checador Toast), Costo Primo (Prime Cost = Comida + Labor), Gastos Operativos Fijos de Sucursal (\`store_operating_expenses\`: Renta, CAM, Servicios Públicos/Utilities, Reparaciones/R&M, Insumos de Tienda, Seguros) y Gastos Corporativos Compartidos Prorrateados (\`shared_brand_expenses\`: Meta Ads / Marketing Digital).
    * **Eliminación Intercompañía de La Bodega (Modo Tienda vs Modo Corporativo)**: Selector crucial que resuelve el dolor financiero identificado en la reunión con Restaurant365:
      - **Modo Tienda (Store View)**: Muestra el P&L individual con el costo facturado por La Bodega a cada sucursal (incluyendo el markup interno de bodega ~10% en carnes y suministros). Útil para evaluar la disciplina operativa de cada gerente.
      - **Modo Corporativo (Corporate Consolidated View)**: Elimina automáticamente el markup interno de La Bodega (~10%), unificando el costo de comida al costo real pagado a proveedores externos (QuickBooks Purchase Cost). Esto evita la doble contabilidad y revela el verdadero EBITDA y margen neto del grupo empresarial consolidado (incrementando la utilidad corporativa en $5,000-$15,000/semana).
    * **Distribución de Gastos de Marca (Meta Ads & Marketing)**: Pestaña interactiva para registrar campañas publicitarias y repartirlas entre tiendas de forma equitativa (División Igualitaria) o ponderada por volumen de ventas (Ponderado por Ventas / Sales-Weighted).
    * **Exportación Ejecutiva CSV**: Botón de descarga de 1 toque con todo el Estado de Resultados comparativo para presentación a socios, directores (Roberto y Erick Velazquez) o contabilidad.
28. **VARIANZA TEÓRICO VS REAL ($ LOSS / PÉRDIDA EN DÓLARES) (/admin/food-cost/varianza)**:
    * **Pestaña 4 en Food Cost**: Replicación exacta del reporte estrella de Restaurant365 (ejemplo Monterey Jack Cheese).
    * **Priorización por Pérdida Financiera**: Clasifica los ingredientes de mayor a menor pérdida en dólares ($ Pérdida = (Uso Real - Uso Teórico) × Costo Unitario).
    * **Foco Inmediato en Fugas**: A diferencia de reportes porcentuales que confunden a la gerencia, la tabla en dólares identifica instantáneamente dónde se está perdiendo dinero real (porciones excesivas, merma no registrada o sustracción de inventario) en carnes principales (Asada, Pastor, Pollo), quesos y aguacate.



SM TEG SIDEBAR NAVIGATION MAP & PATHS (MASTER DIRECTORY - 6 GROUPS):
...
2.  **FINANZAS Y CONTABILIDAD (Finance & Accounting Group - Solo Admin / Raquel)**:
    *   **P&L Multi-Sucursal (Comparative Store-by-Store P&L)**: \`/admin/pnl\`. Access: Admin only (Carlos, Raquel, Roberto, Erick). Purpose: Comparative 15-store P&L side-by-side with Bodega intercompany elimination toggle (Store View vs Corporate View), fixed store operating expenses, Meta Ads shared expense proration, and instant CSV export. Tooltip: Estado de Resultados comparativo multi-tienda con eliminación de markup interno de Bodega y prorrateo de Meta Ads.
    *   **Contabilidad (Accounting / Cohesion Replacement)**: \`/contabilidad\`. Access: Admin only (Carlos, Raquel). Purpose: Daily sales journal entries (pólizas) generated from Toast API and published to QuickBooks Online. Replaces Cohesion (\$450/mes). Tooltip: Daily sales journal entries for QuickBooks — replaces Cohesion.
    *   **Auditoría RONOS (Labor)**: \`/admin/ronos\`. Access: Admin only. Purpose: Automated RONOS (Cingular HR) API timecard auditor, AWS S3 clock photo inspector, California 5th-hour meal penalty leakage calculator, PEO billing reconciliation, and 15-store compliance leaderboard. Arquitectura modular desacoplada (\`components/ronos/\`: RonosHomeTab, StoreAttendanceTab, PayrollReconciliationTab), asignación dinámica de supervisores vía Supabase (\`supervisor_payroll_assignments\`) con separación estricta entre territorio operativo (\`operational_store_id\`) y pagador (\`payroll_store_id\`), soporte i18n 100% bilingüe y conciliación documental de facturas Cingular. Tooltip: Auditoría en vivo de ponchadas, fotos de reloj checador, penalizaciones de lunch y conciliación multi-factura Cingular HR.
    *   **Miles (Acceso Directo Contabilidad / Raquel)**: \`/miles\`. Access: Admin only. Purpose: Direct shortcut to supervisor mileage logs, drive calculations (\$0.760/mi IRS rate), travel reports, and payroll reimbursement dispatch for Raquel and corporate accounting. Tooltip: Control y cálculo de millas manejadas por supervisores, registro de viajes y despacho a RRHH.
    *   **Reporte de Actividades (Activity & Hours Reports)**: \`/admin/reporte-actividades\`. Access: Admin only. Purpose: Executive dashboard with audited dev hours, Lynwood #14 real shift linking, 24h master scale Gantt timeline, 27-task roadmap, and instant CSV export. Tooltip: Interactive executive report of monthly roadmaps, audited dev hours, and Gantt schedules.
3.  **OPERACIONES DE TIENDA (Store Operations Group)**:
...
4.  **INVENTARIO Y MERCANCÍA (Inventory & Merchandise Group)**:
    *   **Radar de Precios (Supplier Price Radar)**: \`/admin/precios-proveedores\`. Access: Admin only. Purpose: Automated vendor API price scraper (Viele & Sons v3), weekly cron inflation detection, 15-store COGS impact calculator, and recipe cascade approval. Tooltip: Automated vendor API scraper, inflation detection, and 15-store COGS audit.
    ...
    *   **Food Cost & Márgenes**: \`/admin/food-cost\`. Access: Admin, Manager, Supervisor. Purpose: Unified master module for food cost. Contains 4 internal tabs: Tab 1 General Store Food Cost (\`/admin/food-cost\`), Tab 2 Meat Yields & Consumption (\`/admin/food-cost/meats\`), Tab 3 Menu Item Margins (\`/inventory/costs\`), Tab 4 Varianza AvT (\$ Pérdida) (\`/admin/food-cost/varianza\`). Tooltip: Comprehensive food cost analysis, meat yields, menu item margins, and Monterey Jack-style Actual vs Theoretical dollar loss variance.
    ...
    *   **Viele & Sons**: \`/admin/compras/viele\`. Access: Admin, Manager, Supervisor. Purpose: Weekly supply order worksheet, printable 2-page count sheets, Sage 100 automated checkout with dual invoice support (sodas vs general supplies), and order history audit. Subpaths: \`/admin/compras/viele/print-sheet\` (Printable count sheet), \`/admin/compras/viele/historial\` (Order history audit). Tooltip: Hoja de pedido semanal Viele & Sons, formato imprimible de conteo y checkout automático con facturación dual.


5.  **PERSONAL Y HORARIOS (Staff & Schedules Group)**:
    *   **Horarios de Tienda (Schedules)**: \`/horarios\`. Access: Manager, Supervisor, Admin. Purpose: View and publish store weekly employee shift schedules. Features intelligent dual UI: Full 14-day desktop shift matrix with drag-and-drop, and dedicated touch-optimized mobile experience with Day-by-Day carousels, store filters, AM/PM traffic light coverage status, and 1-tap shift cards. Tooltip: Creation and publishing of weekly restaurant employee schedules.
...
6.  **ADMINISTRACIÓN Y SISTEMA (System & Admin Group)**:
    ...
    *   **Salud del Sistema (System Health Sentinel)**: Monitoreo continuo de infraestructura Supabase y Vercel (\`/api/cron/monitor-system-health\`). Vigila almacenamiento en disco vs límite de Spend Cap (8 GB), alerta preventivamente a los 6.0 GB y en emergencia a los 7.2 GB, monitorea latencia y disponibilidad, y notifica por correo a carlos@tacosgavilan.com. Herramienta AI: \`check_system_health\`.
...
## MILESIQ (SUPERVISOR MILEAGE TRACKING & PAYROLL REIMBURSEMENT)
- **Módulo Principal (/miles)**: Control de millas recorridas por supervisores de zona para cálculo de reembolsos y despacho a Nómina/RRHH.
- **Tarifa Oficial IRS**: La tasa estándar de reembolso es estrictamente $0.760 USD por milla (rate_per_mile y supervisor_mileage_settings).
- **Destinos Flexibles y Presets Rápidos**: Además de las 15 sucursales oficiales de Tacos Gavilan, Bodega Central y Oficina Corporativa (17 ubicaciones canónicas con factor de desvío de tráfico de LA de 1.33x), soporta destinos libres con chips rápidos: Home Depot / Ferretería, Restaurant Depot / Costco, Banco / Depósito, Clínica / Salubridad, Trámites / Permisos, y tags de propósito (Mantenimiento, Suministros Urgentes, Depósito, Salubridad, Auditoría).
...
## VIELE & SONS PROCUREMENT & DUAL-CHECKOUT ENGINE (/admin/compras/viele)
- **Módulo de Compras Viele & Sons**: Gestión integral de pedidos de insumos secos, desechables, químicos y jarabes de bebidas para las 15 sucursales de Tacos Gavilan.
- **Fórmula Operativa Oficial**: ORDER = MAX(0, PAR - SOBRANTES).
- **Días de Entrega**: Los envíos siempre se programan automáticamente para el próximo martes hábil (getNextTuesday), permitiendo ajustes por emergencias o cierres de sucursal.
- **Comprador Oficial (Customer PO)**: El código registrado por defecto en el 90%+ de las compras de la cadena es "AFV", con soporte para códigos secundarios autorizados (LEWIS, MARK, PV).
- **Arquitectura de Doble Facturación Consecutiva**: Invariablemente se generan dos órdenes separadas y consecutivas en Sage 100 y ClearNine: Factura 1 (Sodas Bag-in-Box 5 gal: BCLCO, BDICO, BSPRI, BMMLE, BMMOR, BSTRA, BRATE, BZECO, BDRPE) sujeta a impuestos de bebidas/CRV, y Factura 2 (Insumos Generales & Desechables) exenta de Sales Tax bajo la Regulación 1589 de California (Food Packaging Exemption).
- **Facturas de Químicos (Imperial Chemical / 'CHEMICAL')**: Químicos de limpieza (jabón de loza IC5GLIDI en bin 2801B, descalcificador IC4DESC en bin 2806B, cloro, desengrasantes) son 100% gravables con California Sales Tax (TX) y se almacenan en el pasillo 28 para prevenir contaminación cruzada de alimentos. Las visitas técnicas de lavavajillas de "Mark" generan facturas independientes de servicio bajo referencias como MARK / CHEMICAL o AFV / CHEMICAL con el vendedor D. TAMAYO y ruta W08.
- **Motor de Checkout de 2 Pasos Fiel al Navegador (ClearNine API)**: El sistema replica exactamente el flujo HTTP de Google Chrome en Windows para que en los servidores de Viele & Sons sea indistinguible de una compra manual en su sitio web:
  1) Limpieza preventiva del carrito (DELETE /api/v3/shopping_cart).
  2) Inyección de líneas de producto (POST /api/v3/shopping_cart).
  3) Carga de formulario real (GET /checkout/) para obtener rowKey[] dinámicos de Sage 100, dirección y ruta de entrega.
  4) Paso 1 de validación idéntico al formulario HTML (POST /checkout/ con validate_main=validate_main, CustomerPO limitado a 15 caracteres, ShipDate en formato MM/DD/YYYY, ShippingMethodID="DELIVERY" y OrderComments).
  5) Validación de pantalla "Check your order!".
  6) Paso 2 de confirmación definitiva (POST /checkout/ con validate_confirm=validate_confirm), emitiendo el número oficial Wxxxxxx de Sage 100 sin disparar alertas ni flags en la infraestructura del proveedor.
  7) Limpieza higiénica final del carrito.
- **Integración Unificada con Radar de Precios & Cron Job (/api/cron/sync-supplier-prices)**:
  - El catálogo de compras \`viele_items\` está 100% sincronizado con el Radar de Precios de Proveedores y el cron job diario (Lunes a Viernes 6:00 AM PST / 13:00 UTC).
  - Columnas de inteligencia en \`viele_items\`: \`previous_price\`, \`price_change_percent\`, \`price_changed_at\`, \`price_status\` ('increased' | 'decreased' | 'unchanged' | 'new'), \`last_scanned_at\`.
  - Detección automática en el cron y bajo demanda (/api/viele/sync-prices):
    * Variaciones de precio: Compara precio de caja de Viele con \`unit_price\`; si varía por > $0.009, actualiza precio, preserva el anterior y calcula el porcentaje.
    * Nuevos productos: Si Viele agrega un SKU a su Order Guide, se da de alta automáticamente en \`viele_items\` con status 'new', imagen y categoría inferida, y se vincula en \`supplier_item_mappings\`.
    * Sincronización de Order Guides por Tienda (\`syncVieleOrderGuides()\`): En el cron diario, recorre las 15 sucursales, extrae el Order Guide de cada cuenta en Viele & Sons con su posición (\`DisplayOrder\`), actualiza \`viele_store_sort_orders\` e inserta artículos nuevos detectados para cada tienda (ej. \`PFXLVIBLK\`).
    * Semáforo en UI: Insignias rojas (aumentos), verdes (rebajas) y azules (nuevos) con tooltips que detallan el precio anterior y fecha de detección.
    * Enlace Cruzado: Acceso directo bidireccional entre /admin/compras/viele y /admin/precios-proveedores.
- **Regla Fiscal Exención de Alimentos & Validación Sage 100**:
  - Transparencia Fiscal Pre-Envío: La UI de captura no inventa ni suma impuestos ficticios (se eliminó el 9.5% estimado). Muestra "Tax pendiente de confirmación por V&S" y el total estimado es exactamente la suma de subtotal de productos.
  - Asignación de Tax Real Post-Checkout: Al confirmarse el pedido en Viele & Sons, el sistema extrae el tax oficial confirmado por Sage 100 ($0.00 para sodas y el tax real para insumos generales).
  - Validación Cruzada Post-Checkout: Consulta inmediata al endpoint \`salesOrderDetail_dt\` de Sage 100 para comparar cada código de producto y cantidad enviada contra los registros de Sage, previniendo contaminación accidental entre sucursales y garantizando exactitud al centavo.`;
