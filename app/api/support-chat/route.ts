...
11. **FORECASTING & SALES INTELLIGENCE v3.1 (TICKET-BASED & EVENT INTELLIGENCE)**: Motor híbrido guest-centric basado en transacciones. Fórmula principal: projectedTotal = baseTickets × ticketGrowth × medianRecentAvgCheck × eventMultiplier × weatherFactor. Unifica order_count, total_tickets y hourly_tickets con fallback seguro; calcula el ticket promedio mediante mediana de días válidos; limita crecimiento de tráfico a 0.92–1.15; y publica un rango de confianza junto con el valor central. La curva usa exclusivamente el día laboral 6:00 AM–5:59 AM, respeta apertura/cierre después de medianoche, completa huecos antes de normalizar y garantiza que la suma horaria sea igual al total diario congelado. Los comparativos de feriados solo usan fechas anteriores. Event Intelligence aplica decaimiento continuo por distancia, neutraliza eventos locales sin ubicación verificable y combina eventos de forma independiente del orden. El clima solo se aplica dentro de 0–7 días calendario de America/Los_Angeles. Las generaciones requieren autenticación y managers solo pueden operar su tienda. Use 'query_forecast' para proyecciones y 'query_upcoming_events' para eventos.
...
22. **AUDITORÍA LABORAL RONOS & FACTURACIÓN CINGULAR HR** (\`/admin/ronos\`): Módulo corporativo de auditoría de nómina, ponchadas y cumplimiento de leyes laborales de California (IWC Wage Order 5 / Labor Code § 512) conectado directamente a la API REST v2.0 de RONOS y sincronizado permanentemente en Supabase (\`ronos_work_weeks\`, \`ronos_employee_timecards_cache\`, \`ronos_transfers_history\`, \`ronos_employee_mappings\`). Cubre el 100% de la empresa: **16 ubicaciones activas** (15 restaurantes + La Bodega Central bajo RONOS Company #28 / Simplify HR site Vernon). Cuenta con 4 pestañas operativas diseñadas con navegación de tarjetas segmentadas de alto contraste: 1) **Mi Tienda** (Auditoría detallada por sucursal: tarjetas de tiempo, horas regulares, horas extras OT 1.5x/DT 2.0x, fotos del reloj checador AWS S3, analizador de Meal Penalties y tarjetas rotas), 2) **Todas las Tiendas** (Comparativo ejecutivo de las 16 ubicaciones para la semana seleccionada con ranking de horas totales, overtime, multas de comida reales, costo de fuga en $ USD, porcentaje de cumplimiento legal y botón 'Ver Tienda' para navegación inmediata), 3) **Vincular Empleados** (Vinculación Toast-RONOS, detección de traslados multi-tienda y envío de avisos oficiales de incumplimiento por correo a la escalera de mando), y 4) **Facturación y Nómina (Motor de Reconciliación PEO)**: Conciliación matemática al centavo contra facturas oficiales de Cingular HR (ej. invoice-TEGW-0009.pdf, TEGB-0016.pdf, TEGS-0032.pdf, TEGL-0022.pdf, TEGH-0009.pdf). Diferencia personal Asalariado/Exempt (General Manager / Supervisor de Zona con salario fijo 80h bisemanal y markup ~24.51%) vs Por Hora/Non-Exempt (Asistente hacia abajo con 26.00% markup sobre tarifa base: BILL_RATE = PAY_RATE * 1.26). Regla de Overtime: OT_BILL_RATE = BILL_RATE * 1.5. Extrae automáticamente horas de Enfermedad (Sick Pay), Vacaciones (PTO Vacation) y Feriados (Holiday) capturados en las tabletas RONOS en tienda (workDays vacationHours y sickHours), totalizando salarios brutos (TOT PAY), margen Cingular (Cingular Fee) y facturación total (TOT BILL) con exportación a CSV idéntico al Summary Report oficial. Integra sincronización masiva y automática con Simplify HR OS mediante login corporativo de Raquel (\`raquel@tacosgavilan.com\`, rol \`shr_hrproxy\`) y consulta de Paystubs oficiales con MongoDB ObjectIDs de 24 caracteres (\`getSitePaystubs\`), capturando automáticamente ausencias de enfermedad (Sick Leave) o vacaciones (PTO) aprobadas administrativamente fuera del reloj checador de tienda (como las 8h Sick de Erasto Barranco en Hollywood TEGH-0009) y sincronizando salarios y tarifas al centavo sin requerir ajustes manuales. Omitir placeholder del sistema 'Manager Default' y cuentas de control técnico PIN 1111. Blindaje null-safe completo en 73 puntos de vulnerabilidad.
...
2.  **FINANZAS Y CONTABILIDAD (Finance & Accounting Group - Solo Admin / Raquel)**:
    ...
    *   **Auditoría RONOS (Labor)**: \`/admin/ronos\`. Access: Admin only. Purpose: Automated RONOS (Cingular HR) API timecard auditor, AWS S3 clock photo inspector, California 5th-hour meal penalty leakage calculator, PEO billing reconciliation, and 15-store compliance leaderboard. Incluye la Pestaña 4 de Facturación y Nómina con Bloque Maestro de Salida de Caja (Cash Outflow Master Card), Selector de 3 Modos (Consolidado Total Tienda, Factura Ordinaria, Finiquitos Separados), y detección automática de facturas complementarias/finiquitos con sincronización diaria a las 11:59 AM PT (/api/cron/sync-daily-payroll). Tooltip: Auditoría en vivo de ponchadas, fotos de reloj checador, penalizaciones de lunch y conciliación multi-factura Cingular HR.
...
- **Integración Unificada con Radar de Precios & Cron Job (/api/cron/sync-supplier-prices)**:
  ...
  - Detección automática en el cron y bajo demanda (/api/viele/sync-prices):
    ...
    * Widget de Cabecera: Cápsula informativa con última auditoría, conteo de alertas, filtro de variaciones y botón de sincronización bajo demanda en 1 clic.
