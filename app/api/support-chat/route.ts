...
23. **Contabilidad (Accounting / Cohesion Replacement)**: Módulo que reemplaza la plataforma Cohesion ($450/mes). Genera pólizas de ventas diarias (journal entries) para cada sucursal usando datos de Toast API y las publica a QuickBooks Online. Flujo: Centinela Automático de las 6:15 AM PST (/api/cron/sync-accounting) que corre tras el corte de las 5:59 AM, procesa el día de ayer y ejecuta una **Ventana Rodante de Auditoría Continua de 7 Días** (días -2 a -7). Si un día previo aún no está publicado, lo recalcula automáticamente ante cambios tardíos en Toast POS. Si ya estaba publicado en QuickBooks y alguien aplica un reembolso tardío en tienda (ej. $3.99 o $7.98), el sistema lo detecta sin alterar QuickBooks y genera una Alerta de Discrepancia Post-Publicación en el dashboard para que Raquel genere un Asiento de Ajuste con 1 clic. Cada póliza tiene ~18 líneas de débito/crédito que mapean ventas por canal (For Here, To Go, Drive Thru, Toast Online, Uber, DoorDash, GrubHub), impuestos (Sales Tax, Marketplace Facilitator, Tax Paid by Uber), pagos (tarjetas de crédito, EBT, third-party), y efectivo. Estados: pending→ready→reviewed→published. La tabla accounting_site_mappings define los mapeos GL por tienda (cuenta de banco, ubicación QB, clase QB). Incorpora el panel interactivo de configuración /contabilidad/configuracion con modal de edición deslizable para cada una de las 15 sucursales, permitiendo a Raquel y Carlos editar ubicaciones QB, clases, cuentas bancarias de depósito, mapeos GL de ventas/impuestos/cobranzas, o restaurar los valores oficiales de Cohesion en tiempo real. Incorpora la regla de oro de validación del **Step 11 de Cohesion** (*Check for Open OR Out-of-Balance Orders*): si Toast POS detecta órdenes abiertas o checks sin cerrar/desbalanceados, el sistema bloquea estrictamente la publicación a QuickBooks Online con código HTTP 422, muestra un Banner de Alerta Ámbar en /contabilidad/[packetId] con tabla interactiva de órdenes pendientes (número, cajero, monto y motivo), el botón de publicar se deshabilita y se despliega el badge de alerta en la vista principal. Modo de Publicación: Opción A Semiautomática (el centinela deja todo listo en verde 'ready' a las 6:15 AM y Raquel da 1 clic en "Publicar Todo el Día").
24. **SISTEMA Y VERSIÓN V2.8.0**: La versión canónica actual del sistema es **SM TEG v2.8.0** (Producción • Septiembre 2026), visible prominentemente con insignia verde esmeralda y pulso de actividad en el menú de perfil de usuario (esquina superior derecha y menú lateral móvil). Incluye los hitos de Cohesion (v2.6.1), Centro de Configuración Contable (v2.6.3), Motor de Compras Viele & Sons con Sage 100 dual (v2.7.0), Sincronización de Ausencias RONOS y Puente Planificador-Horarios (v2.7.5), y Centinela de Salud del Sistema con Blindaje PMIX y Dual-Gate Crons (v2.8.0).
