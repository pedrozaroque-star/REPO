...
8.  **INVENTORY**: Items, recipes, menu catalog, costs per unit. Sincronización con QuickBooks protege precios mediante Smart Price Protection (bloquea caídas >= max_drop_percent, default 50%). Auto-sync de empaques lee el campo Description de QB. Protección de piezas: items por pieza (pza/unit/dz) nunca son sobrescritos a peso/volumen por el sync. NUEVO: Edición Inteligente de PAR por Día en Configuración Semanal — si un día ya tiene sobrante capturado o ya pasó, editar su PAR se guarda exclusivamente para la Próxima Semana para no alterar el histórico ni los porcentajes de sobrante de esta semana; si el día no tiene sobrante aún, se actualiza de inmediato tanto la semana actual como la próxima. Para líquidos y uniformes, calculateDailyOrder evalúa siempre de forma inmutable el PAR de mon_par en cualquier día de la semana.
...
4.  **INVENTARIO (Inventory Group)**:
    ...
    *   **Preparador (Prep Tool)**: \`/inventory/preparador\`. Access: Admin, Manager, Supervisor, Assistant (asistente). Purpose: Production projections, grill cooking pace, and raw meat historical consumption curves. Features: (1) 30-Minute Interval Cards (Tarjetas por cada media hora, e.g. 5:00pm a 5:30pm) displaying raw meat needed on the grill, with a view switcher to Tramos de Tiempo (Peak Period Blocks, e.g. HORA PICO PM 5:00pm a 9:00pm). (2) Live 3-minute polling interval with \`cache: 'no-store'\` headers and timestamp cache-busters to keep all devices uniformly synchronized with live Toast sales. (3) 3-month rolling DOW historical baseline for meat projections (\`get_meat_history_avg\`). (4) Dual tablet support (Line Cook & Back Kitchen / Bodega). (5) Haptic vibration alerts before period ends and screen Wake Lock. (6) Interactive calendar date picker to view baseline projections and real historical sales for any specific date.
    *   **Orden diaria (Bodega Orders)**: \`/inventory/orders\`. Access: Admin, Manager, Supervisor. Purpose: Roster store-level daily orders to La Bodega and synchronize as QuickBooks estimates. Features: (1) Daily orders, weekly configuration (BASE/PAR settings), history logs, and daily leftovers matrices. (2) Live background auto-sync from QuickBooks Online on page load or tab/store switch. Auto-resolves missing QB items and updates store_order_template automatically. Includes live "Packaging (QB)" (e.g. bag of 5 lbs) and "Cost (QB)" columns in order sheets and edit modals. (3) The weekly config view has a right-side toggle switch (defaulting to Leftovers) to dynamically compare daily PAR values on the left against either weekly leftovers (orange) or suggested Ideal PAR (violet) side-by-side. (4) Adjustments made to PAR bases save to the following week (+7 days) to protect current orders. (5) QB packaging sync includes an ignore list for Milanesa, Salchicha, Pastor, and Pollo to prevent automated PAR scaling spikes. (6) Programmatic trailing-slash sanitization on \`QUICKBOOKS_REDIRECT_URI\` to prevent \`invalid_grant\` errors. (7) Order Edit Restriction: Orders sent to QB can only be edited on their creation date (\`order_date == today\`). Past orders open in Read-Only consultation mode ("📋 Consultar Pedido"). (8) Extraordinary Items Combo: Emergency extra items search dropdown is integrated directly inside the Edit Modal to allow adding extra supplies on the fly.
