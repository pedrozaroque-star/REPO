...
25. **COMPRAS E INVENTARIO VIELE & SONS (/admin/compras/viele)**: Módulo integral de compras, conteo y pedidos semanales conectado al portal ClearNine / Sage 100 de Viele & Sons (\`shop.vieleandsons.com\`).
    ...
    * **REGLAS DE FACTURACIÓN Y TIPOS DE INVOICES (Sodas, Insumos y Químicos)**: Viele & Sons genera facturas separadas debido a regulaciones fiscales y divisiones de almacén:
      ...
      Al hacer checkout en la app, si el pedido contiene sodas e insumos, el sistema ejecuta AUTOMÁTICAMENTE 2 órdenes consecutivas e independientes en Viele & Sons (Orden 1: Sodas -> Wxxxxxx, Orden 2: Insumos -> Wxxxxxx correlativa), enlazándolas en \`viele_orders\` con \`order_category\` y \`linked_order_number\`.
    ...
    * **ARQUITECTURA DE SEGURIDAD, RBAC Y RESILIENCIA DE CHECKOUT (Auditoría 2026-09-21)**:
      1) **Aislamiento de Secretos (Zero Client Exposure)**: Credenciales de acceso de las 15 tiendas protegidas estrictamente en el backend con \`server-only\` (\`lib/viele-credentials-server.ts\`), exponiendo al frontend únicamente IDs y nombres públicos (\`lib/viele-stores-public.ts\`).
      ...
      4) **Resiliencia ante Éxito Parcial (Split Orders)**: Si la orden de Sodas es confirmada en Viele pero Insumos falla, Sodas se persiste inmediatamente en Supabase con HTTP 207 Multi-Status y las sodas se descuentan del carrito para que el usuario reintente únicamente los insumos generales sin duplicar sodas.
