...
18. **UNIFORMS CONTROL**: Module to track uniform inventory, shipments reception from Bodega Orders (QuickBooks estimates), deliveries, and sales to employees. Standard prices: Shirts $7, Caps $1, Jackets $20. Black Shirts/Polos are free for managers and leadership. Standard minimum stock levels per garment and size: Team Members Red (XS:10, S:10, M:20, L:15, XL:15, 2XL:5, 3XL:5), Red Caps (12), Red Jackets (XS-XL:3, 2XL-3XL:2), Shift Leader Black (XS:10, S:10, M:20, L:15, XL:15, 2XL:5, 3XL:5), Assistant Manager Polo Black (XS-3XL:3), Black Caps (12), Black Jackets (XS-XL:3, 2XL-3XL:2). When stock reaches or falls below target, a prominent warning alert banner is triggered with direct access to the Bodega Replenishment Modal (ReorderModal) to request stock renewal from La Bodega. Flexible delivery/new hire package ($0.00): customizable torso garment (Red Team, Shift Leader Black, Assistant Manager Polo Black, Store Manager Black Shirt), custom quantity with strict dynamic stock clamping (\`max={stock}\`), optional cap (Red vs Black), and optional jacket (Red for Team Members, Black for Shift Leader and above). Damage exchanges support custom replacement quantity input. Admin controls (Edit Prices, Reset Initial Count, Audit Mode) are restricted strictly to admins (\`user.role === 'admin'\`). Order Reception tab loads uniform orders sent from Bodega Orders (\`order_type = 'uniforms'\`), parses QuickBooks item names to category/size, and updates physical stock upon confirmation. Cash sales flow directly into Safe Management (Caja Fuerte) daily reconciliation via API \`GET /api/inventory/uniforms/safe-reconciliation\`.
