...
function SobranteContent() {
  ...
  const { data: template, loading: checklistLoading, error: checklistError, isCached } = useDynamicChecklist('sobrante_v1')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const missingAnswers = questions.filter(q => {
      ...
      if (q.type === 'number') return !val || val.trim().length === 0
      if (q.type === 'photo') return !questionPhotos[q.id] || questionPhotos[q.id].length === 0
      return val === undefined || val === null
    })

    if (missingAnswers.length > 0) {
      alert(`⚠️ Faltan ${missingAnswers.length} productos por reportar.`)
      return
    }
    ...
    if (missingRequiredPhotos.length > 0) {
      alert(`📸 Hay ${missingRequiredPhotos.length} reportes que requieren foto obligatoria.`)
      return
    }
    ...
    return (
      <div className="min-h-screen bg-transparent grid place-items-center animate-in zoom-in duration-500">
        <div className="text-center p-8 bg-white rounded-3xl shadow-2xl border border-gray-100 max-w-sm mx-auto">
          ...
          <h1 className="text-4xl font-black text-gray-900 mb-2">¡Reporte Enviado!</h1>
          <p className="text-gray-500 font-medium mb-8">El sobrante de producto ha sido registrado.</p>
      ...
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-200 dark:border-slate-800 sticky top-0 z-40 shadow-sm shrink-0 transition-all">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-black text-gray-900 dark:text-white flex items-center gap-2">
                {template?.title || 'Reporte de Sobrante'}
                {isCached && (
          ...
          <div className="bg-orange-600 text-white px-4 py-2 rounded-2xl shadow-lg shadow-orange-100 flex flex-col items-center min-w-[80px]">
            <div className="text-xl font-black leading-none">{answeredCount}/{questions.length}</div>
            <div className="text-[10px] font-bold uppercase opacity-80 mt-1">PRODUCTOS</div>
          </div>
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-3xl shadow-sm p-6 border border-gray-100 dark:border-slate-800 grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <select required value={formData.store_id} onChange={(e) => setFormData({ ...formData, store_id: e.target.value })}
                className="w-full p-3 bg-gray-50 dark:bg-slate-800 border-gray-100 dark:border-slate-700 rounded-xl font-bold text-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-orange-100 dark:focus:ring-orange-900/20 transition-all">
            ...
            {template?.sections.map((section: any) => (
              <div key={section.id} className="space-y-6">
                <div className="flex items-center gap-4 px-2">
                  <div className="h-[2px] flex-1 bg-gray-100 dark:bg-slate-800" /> {/* 3. Update header and cards for dark mode */}
                  <h2 className="text-[10px] font-black text-orange-600 dark:text-orange-500 uppercase tracking-[0.3em]">{section.title}</h2> {/* 3. Update header and cards for dark mode */}
                  <div className="h-[2px] flex-1 bg-gray-100 dark:bg-slate-800" /> {/* 3. Update header and cards for dark mode */}
                </div>
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-3xl shadow-sm p-6 border border-gray-100 dark:border-slate-800 space-y-3">
            ...
            <textarea value={formData.comments} onChange={(e) => setFormData({ ...formData, comments: e.target.value })} rows={3}
              className="w-full p-4 bg-gray-50 dark:bg-slate-800/50 border-gray-100 dark:border-slate-700 rounded-2xl font-medium text-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-orange-100 dark:focus:ring-orange-900/20 transition-all resize-none shadow-inner"
              placeholder="Escribe aquí cualquier observación relevante..." />
          </div>

          <div className="pt-4">
            <button type="submit" disabled={loading}
              className="w-full bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white font-black text-lg py-5 rounded-3xl shadow-xl shadow-orange-100 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3">
              {loading ? <div className="w-6 h-6 border-4 border-white border-t-transparent rounded-full animate-spin" /> : 'Finalizar Reporte'}
            </button>
          </div>
        </form>
      </div>
