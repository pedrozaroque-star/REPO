...
function RecorridoContent() {
  ...
  const { data: template, loading: checklistLoading, error: checklistError, isCached } = useDynamicChecklist('recorrido_v1')
    ...
    return (
      <div className="min-h-screen bg-transparent grid place-items-center animate-in zoom-in duration-500">
        <div className="text-center p-8 bg-white rounded-3xl shadow-2xl border border-gray-100 max-w-sm mx-auto">
          ...
          <h1 className="text-4xl font-black text-gray-900 mb-2">¡Recorrido Listo!</h1>
          <p className="text-gray-500 font-medium mb-8">El reporte de recorrido ha sido registrado.</p>
          <button onClick={() => router.push('/dashboard')} className="w-full py-4 bg-green-600 text-white font-bold rounded-2xl shadow-lg shadow-green-100 transition-transform active:scale-95">
            Volver al Inicio
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-transparent dark:bg-neutral-900 pb-20 font-sans relative overflow-hidden">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg font-black text-gray-900 dark:text-white flex items-center gap-2">
                {template?.title || 'Recorrido de Limpieza'}
                {isCached && (
              ...
              <div className="text-[10px] font-black text-green-600 uppercase tracking-widest">{user.name || user.email}</div>
            </div>
          </div>
          <div className="bg-green-600 text-white px-4 py-2 rounded-2xl shadow-lg shadow-green-100 flex flex-col items-center min-w-[80px]">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 pb-32 w-full">
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-3xl shadow-sm p-6 border border-gray-100 dark:border-slate-800 grid grid-cols-1 lg:grid-cols-3 gap-6 sticky top-0 z-30">
            <div className="space-y-2">
              ...
              <select required value={formData.store_id} onChange={(e) => setFormData({ ...formData, store_id: e.target.value })}
                className="w-full p-3 bg-gray-50 dark:bg-slate-800 border-gray-100 dark:border-slate-700 rounded-xl font-bold text-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/20 transition-all">
            ...
            {template?.sections.map((section: any) => (
              <div key={section.id} className="space-y-6">
                <div className="flex items-center gap-4 px-2">
                  ...
                  <h2 className="text-[10px] font-black text-green-600 dark:text-green-500 uppercase tracking-[0.3em]">{section.title}</h2>
                  <div className="h-[2px] flex-1 bg-gray-100 dark:bg-slate-800" />
                </div>
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm rounded-3xl shadow-sm p-6 border border-gray-100 dark:border-slate-800 space-y-3">
            <label className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">Observaciones del Recorrido</label>
            <textarea value={formData.comments} onChange={(e) => setFormData({ ...formData, comments: e.target.value })} rows={3}
              className="w-full p-4 bg-gray-50 dark:bg-slate-800/50 border-gray-100 dark:border-slate-700 rounded-2xl font-medium text-gray-700 dark:text-white outline-none focus:ring-2 focus:ring-green-100 dark:focus:ring-green-900/20 transition-all resize-none shadow-inner"
              placeholder="Escribe aquí cualquier observación relevante..." />
          </div>

          <div className="pt-4">
            <button type="submit" disabled={loading}
              className="w-full bg-gradient-to-r from-green-600 to-emerald-700 hover:from-green-700 hover:to-emerald-800 text-white font-black text-lg py-5 rounded-3xl shadow-xl shadow-green-100 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3">
              {loading ? <div className="w-6 h-6 border-4 border-white border-t-transparent rounded-full animate-spin" /> : 'Finalizar Recorrido'}
            </button>
          </div>
        </form>
      </div>
