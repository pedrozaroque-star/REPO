...
function ChecklistsContent() {
  ...
  const fetchStores = async () => {
    try {
      ...
      // --- 🔒 FILTRO DE SEGURIDAD (Selector) ---
      // Si el usuario es asistente, solo mostramos sus tiendas en el combo
  ...
  const fetchData = async () => {
    try {
      ...
      // --- 🔒 FILTRO DE SEGURIDAD (Base de Datos) ---
      // Forzamos a la base de datos a devolver SOLO lo que el usuario tiene permitido ver
      const userScope = (user as any)?.store_scope
      if (user?.role === 'asistente') {
          if (Array.isArray(userScope) && userScope.length > 0) {
            // Convertir códigos/nombres a IDs numéricos
            const allowedStoreIds = stores
              .filter(s => userScope.includes(s.code) || userScope.includes(s.name))
              .map(s => s.id)
            
            if (allowedStoreIds.length > 0) {
              // Sintaxis Supabase: store_id=in.(1,2,3)
              checkUrl += `&store_id=in.(${allowedStoreIds.join(',')})`
            } else {
              // Si no hay tiendas permitidas, no ve nada
              checkUrl += `&store_id=eq.0`
            }
          } else {
            // Si es asistente y no tiene tiendas asignadas, no ve nada (seguridad)
            checkUrl += `&store_id=eq.0`
          }
      }
      ...
      // Stats (También filtramos para que los contadores sean reales)
      let allCheckUrl = `${url}/rest/v1/assistant_checklists?select=checklist_type`
      if (user?.role === 'asistente' && Array.isArray(userScope) && userScope.length > 0) {
         // Convertir códigos/nombres a IDs numéricos para stats
         const allowedStoreIds = stores
           .filter(s => userScope.includes(s.code) || userScope.includes(s.name))
           .map(s => s.id)
