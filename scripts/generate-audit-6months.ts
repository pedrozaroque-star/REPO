...
async function generateReport() {
    // 6 MONTHS: Aug 1 2025 - Jan 29 2026
    const startDate = parseISO('2025-08-01');
    const endDate = parseISO('2026-01-29');

    console.log(`\n🌎 GENERANDO REPORTE 6 MESES (Ago 25 - Ene 26)...\n`);

    // --- PRE-FETCH REAL DATA (MASSIVE OPTIMIZATION) ---
    console.log("📥 Descargando historial reciente...");
    ...
    for (const [id, name] of Object.entries(STORES)) {
        ...
        while (currentDate <= endDate) {
            dayCounter++;
            if (dayCounter % 30 === 0) process.stdout.write(`|`); // More frequent updates since period is shorter
    ...
    <head>
        <meta charset="UTF-8">
        <title>Audit Report 6-Months</title>
    ...
    <body class="p-8 max-w-7xl mx-auto">
        ...
        <header class="mb-10 text-center">
            <h1 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-500 mb-2">
                Intelligence v2.1 (Recent Performance)
            </h1>
            <p class="text-slate-400">Period: Aug 1, 2025 - Jan 29, 2026 • 15 Stores</p>
        </header>
    ...
    const outputPath = path.join(process.cwd(), 'public', 'audit-report-6m.html');
    fs.writeFileSync(outputPath, html);
    console.log(`\n✅ REPORTE 6 MESES GENERADO:\n👉 ${outputPath}\n`);
}
