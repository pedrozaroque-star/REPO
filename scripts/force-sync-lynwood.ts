...
async function main() {
    ...
    console.log('1. Force syncing Lynwood labor...')
    ...
    punches.forEach(p => {
        ...
        totalRegular += reg
        totalOvertime += ot
        // Rough cost calc: (Reg + OT*1.5) * Wage? Or just (Reg+OT)*Wage?
        // Toast usually separates them. Let's assume wage is base rate.
        estimatedCost += (reg * wage) + (ot * wage * 1.5)

        // Log suspicious entries (e.g. > 12 hours or Wage > $30)
        if ((reg + ot) > 12 || wage > 30) {
            console.warn(`⚠️ Suspicious Punch: ${p.first_name} ${p.last_name} / Hours: ${reg + ot} / Wage: ${wage}`)
        }
    })
