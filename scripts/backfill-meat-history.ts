...
// Configuración de Entorno Local o Producción
...
function get30MinBucket(isoDateStr: string): string {
    // 1. Convertir fecha UTC a Hora LA
    ...
    // formatter.format devuelve ej: "14:45" o "2:15"
    let timeParts = formatter.format(dateObj).split(':')
    let h = parseInt(timeParts[0], 10)
    let m = parseInt(timeParts[1], 10)
    
    // Si hour era 24 (Midnight), lo ajustamos a 0
    if (h === 24) h = 0;
    
    // 2. Redondear a la media hora inferior (0 o 30)
    m = m >= 30 ? 30 : 0
...
async function run() {
    ...
    recipesData.forEach(r => {
        if (itemLookup.has(r.inventory_item_id)) {
            // Guardamos el Meat Type base para agrupamiento rápido
        ...
        while (hasMore) {
            ...
            maxRetries = 5
                ...
                entries.forEach((order) => {
                    ...
                    const bucketTime = get30MinBucket(openedIso) // ej: "12:30:00"
