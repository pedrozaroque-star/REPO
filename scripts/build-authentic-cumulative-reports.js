...
console.log('═══════════════════════════════════════════════════════════════════════');
console.log('💎 RECONSTRUCCIÓN DEFINITIVA CUMULATIVA CON LAS 26 TAREAS AUTÉNTICAS');
console.log('═══════════════════════════════════════════════════════════════════════');

const raw20Tasks = JSON.parse(fs.readFileSync('scripts/extracted-july-canonical-tasks.json', 'utf-8'));

const master26Tasks = [
  ...raw20Tasks.map((t, idx) => {
    ...
    return {
      ...
      badgeDept: t.dept || '💻 Sistemas',
      badgePriority: num <= 8 ? '🔴 Alta' : num <= 15 ? '🟡 Media' : '🟢 Normal',
      status: num <= 14 ? 'completado' : num <= 18 ? 'progreso' : 'pendiente',
      statusLabel: num <= 14 ? '✓ Completado' : num <= 18 ? '⚡ En Progreso' : '⏳ Pendiente',
      audit: t.audit || '',
      steps: t.steps && t.steps.length ? t.steps : ['Análisis y requerimientos.', 'Desarrollo e integración.', 'Pruebas y despliegue.']
    };
  }),
  // Task 18
  {
    ...
    audit: '<strong>Completado.</strong> Actualización y descarga de la biblioteca de videos musicales de Regional Mexicano (éxitos recientes, corridos, banda, norteñas) en formato MP4 HD organizados en unidades USB para reproducción en los televisores de los restaurantes.',
    steps: [
  ...
  },
  {
    ...
    audit: '<strong>Planificado para Temporada de Frío (Otoño-Invierno).</strong> Modelo de proyección estacional en /api/inventory/champurrado-forecast con 5 años de historial de semanas ISO y sugerencia informativa de galones diarios para la orden de bodega.',
    steps: [
...
// Deduplicate and sort 1..26
const uniqueTasksMap = new Map();
master26Tasks.forEach(t => uniqueTasksMap.set(t.num, t));
const sorted26Tasks = Array.from(uniqueTasksMap.values()).sort((a, b) => a.num - b.num);

console.log(`Total Unique Master Tasks: ${sorted26Tasks.length}`);
sorted26Tasks.forEach(t => console.log(`  ${t.num}. ${t.title} [${t.statusLabel}]`));

// Save to master-tasks-data.js
fs.writeFileSync('scripts/master-tasks-data.js', `module.exports = ${JSON.stringify(sorted26Tasks, null, 2)};`, 'utf-8');
...
// 1. June Tasks (17 tasks)
const juneTasks = JSON.parse(JSON.stringify(sorted26Tasks.slice(0, 17))).map((t, idx) => {
    if (idx < 13) {
        t.status = 'completado';
...
// 2. July Tasks (20 tasks = June 17 + July 3)
const julyTasks = JSON.parse(JSON.stringify(sorted26Tasks.slice(0, 20))).map((t, idx) => {
    if (idx < 14) {
        t.status = 'completado';
        t.statusLabel = '✓ Completado';
    } else if (idx < 18) {
...
// 3. August Tasks (26 tasks = June 17 + July 3 + August 6)
const augustTasks = JSON.parse(JSON.stringify(sorted26Tasks)).map((t, idx) => {
    if (idx < 18) {
        t.status = 'completado';
        t.statusLabel = '✓ Completado';
    } else if (idx < 24) {
...
console.log('✅ Reconstrucción completada para Junio, Julio y Agosto!');

(async () => {
    console.log('📸 Generando PDF de escritorio...');
