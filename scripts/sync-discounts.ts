...
async function syncStoreDiscountsForDate(token: string, storeId: string, storeName: string, dateStr: string, employeeMap: Map<string, string>) {
    ...
    while (hasMore) {
        ...
        orders.forEach(order => {
            ...
            const serverNameOrder = buildName(order.server, 'Caja / Sistema Automático')
    ...
    try {
        ...
        while (currentDate >= targetEndDate) {
            ...
            for (const [storeId, storeName] of Object.entries(TOAST_GUID_MAP)) {
                console.log(`Procesando tienda: ${storeName} para ${dateStr}...`)
                await syncStoreDiscountsForDate(token, storeId, storeName, dateStr, employeeMap)
            }
