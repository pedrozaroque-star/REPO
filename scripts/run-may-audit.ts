...
async function runMayAudit() {
    const startDate = parseISO('2025-05-01');
    const endDate = parseISO('2025-05-31');

    console.log(`\n🌸 AUDITORÍA INTELLIGENCE v2 (Mayo 2025)...\n`);
    ...
    while (currentDate <= endDate) {
        ...
        try {
            ...
            if (real && real.net_sales > 0) {
                ...
                // Highlight Holidays
                if (dateStr === '2025-05-05') icon += ' 🌮'; // 5 Mayo
                if (dateStr === '2025-05-11') icon += ' 💐'; // Mother's Day
                if (dateStr === '2025-05-26') icon += ' 🇺🇸'; // Memorial Day
