export const getTempValidation = (questionText: string, value: number, sectionTitle?: string) => {
    ...
    // Support Spanish and English keywords for cold storage
    const isRefrig = textToCheck.includes('refrig') ||
        textToCheck.includes('frio') ||
        textToCheck.includes('cold') ||
        textToCheck.includes('cooler') ||
        textToCheck.includes('freezer')
