 ...
 *   - Exportación de estimaciones de nómina a CSV para revisión contable.
 *
 * @businessRules
 ...
 *
 * @notes
 *   - La conciliación documental muestra factura, cálculo y diferencias conservando las horas RONOS; coincidencia monetaria no certifica cumplimiento laboral.
 *   - La facturación calculada es estimada; una coincidencia con paystub compara únicamente sueldo bruto, no una factura oficial.
 *   - Las consultas de nómina cancelan respuestas obsoletas y exponen errores sin convertir datos ausentes en cero.
...
import { isEmployeeSalaried } from '@/lib/payroll-classification'
...
interface PayrollInvoiceReconciliation {
  invoiceIds: string[]
  sourceFiles: string[]
...
interface PayrollInvoicePayrollVerification {
  invoiceEmployees: number
  approvedPaystubs: number
  linkedEmployees: number
  grossMatches: number
  grossVariances: number
  unmatchedInvoiceEmployees: number
  unmatchedPaystubs: number
  grossVariance: number
  status: 'matched' | 'variance' | 'incomplete'
}

interface PayrollInvoiceEvidence {
  invoiceId: string
  sourceFile: string
  ...
  grossVariance: number
  billingVariance: number
  regularHoursVariance: number
  overtimeHoursVariance: number
  ptoHoursVariance: number
  rateSource: 'official_invoice'
  identityVerified: boolean
  status: 'matched' | 'variance' | 'incomplete'
}

/** Formatea evidencia sin presentar datos ausentes o no finitos como cero. */
function formatInvoiceValue(value: unknown, currency = true, signed = false): string {
...
/** Comparación documental del período; el motor determina cobertura y coincidencia. */
function PayrollInvoicePanel({ evidence, payrollVerification }: { evidence: PayrollInvoiceReconciliation; payrollVerification?: PayrollInvoicePayrollVerification }) {
  return (
    <section aria-label="Conciliación con factura oficial" className="rounded-xl border border-slate-300 dark:border-slate-700 p-4 space-y-3 bg-white dark:bg-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="font-bold text-sm">Conciliación con factura oficial</h3>
        <span className={`rounded px-2 py-1 text-xs font-bold ${evidence.status === 'matched' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>
          {evidence.status === 'matched' ? 'Importes coinciden con la factura' : evidence.status === 'incomplete' ? 'Cobertura documental incompleta' : 'Diferencias con la factura'}
        </span>
      </div>
      <p className="text-xs text-slate-600 dark:text-slate-300">
        Facturas: {Array.isArray(evidence.invoiceIds) && evidence.invoiceIds.length ? evidence.invoiceIds.join(', ') : 'Sin identificar'}.
        {' '}Diferencia = cálculo − factura; un valor positivo indica un cálculo mayor. La coincidencia de importes no certifica cumplimiento laboral.
      </p>
      <div className="overflow-x-auto">
        <table className="w-full text-xs text-right">
          <thead><tr className="border-b border-slate-200 dark:border-slate-700"><th className="p-2 text-left">Concepto</th><th className="p-2">Factura oficial</th><th className="p-2">Cálculo</th><th className="p-2">Diferencia</th></tr></thead>
          <tbody>
            <tr><th scope="row" className="p-2 text-left font-medium">Sueldo bruto</th><td className="p-2 font-mono">{formatInvoiceValue(evidence.officialGrossPay)}</td><td className="p-2 font-mono">{formatInvoiceValue(evidence.estimatedGrossPay)}</td><td className="p-2 font-mono font-bold">{formatInvoiceValue(evidence.grossVariance, true, true)}</td></tr>
            <tr><th scope="row" className="p-2 text-left font-medium">Total facturado</th><td className="p-2 font-mono">{formatInvoiceValue(evidence.officialBilledAmount)}</td><td className="p-2 font-mono">{formatInvoiceValue(evidence.estimatedBilledAmount)}</td><td className="p-2 font-mono font-bold">{formatInvoiceValue(evidence.billingVariance, true, true)}</td></tr>
          </tbody>
        </table>
      </div>
      <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs">
        <span>Colaboradores vinculados: <strong>{evidence.matchedEmployees ?? 'Sin dato'}</strong></span>
        {typeof evidence.candidateEmployees === 'number' && <span>Candidatos pendientes de identidad: <strong>{evidence.candidateEmployees}</strong></span>}
        <span>En factura sin vínculo RONOS: <strong>{evidence.unmatchedInvoiceEmployees ?? 'Sin dato'}</strong></span>
        <span>En RONOS sin vínculo a factura: <strong>{evidence.unmatchedRonosEmployees ?? 'Sin dato'}</strong></span>
      </div>
      {payrollVerification && <div className={`rounded-lg p-3 text-xs ${payrollVerification.status === 'matched' ? 'bg-emerald-50 text-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-200' : 'bg-amber-50 text-amber-900 dark:bg-amber-950/40 dark:text-amber-200'}`}>
        <strong>Factura ↔ Simplify HR:</strong> {payrollVerification.grossMatches}/{payrollVerification.invoiceEmployees} empleados enlazados por número oficial con bruto al centavo.
        {' '}Diferencia acumulada: {formatInvoiceValue(payrollVerification.grossVariance, true, true)}. Esta verificación no enlaza aún a RONOS.
        {(payrollVerification.unmatchedInvoiceEmployees > 0 || payrollVerification.unmatchedPaystubs > 0) && <span> Pendientes: {payrollVerification.unmatchedInvoiceEmployees} en factura y {payrollVerification.unmatchedPaystubs} recibos.</span>}
      </div>}
      {Array.isArray(evidence.sourceFiles) && evidence.sourceFiles.length > 0 && <details className="text-xs text-slate-600 dark:text-slate-300"><summary className="cursor-pointer font-semibold">Documentos utilizados</summary><ul className="mt-2 space-y-1 list-disc pl-4">{evidence.sourceFiles.map((source, index) => <li key={`${source}-${index}`} className="break-all">{source}</li>)}</ul></details>}
    </section>
  )
}

/** Evidencia por colaborador: importes del documento separados del cálculo RONOS. */
function PayrollInvoiceEmployeeDetail({ evidence, grossPay, billedAmount }: { evidence: PayrollInvoiceEvidence; grossPay: unknown; billedAmount: unknown }) {
  return <details className="mt-2 min-w-[230px] text-left text-xs">
    <summary className="cursor-pointer font-semibold text-sky-700 dark:text-sky-300">{!evidence.identityVerified ? 'Candidato de factura · validar identidad' : evidence.status === 'matched' ? 'Importes coinciden con factura' : 'Ver diferencias con factura'}</summary>
    <div className="mt-2 space-y-2 rounded-lg border border-slate-200 dark:border-slate-700 p-2 bg-slate-50 dark:bg-slate-800">
      <p className="font-semibold">{evidence.invoiceId} · Empleado #{evidence.employeeNumber}</p>
      {!evidence.identityVerified && <p className="font-semibold text-amber-800 dark:text-amber-300">Coincidencia por nombre; esta comparación es provisional hasta verificar la identidad.</p>}
      <table className="w-full text-right text-[11px]"><thead><tr><th className="py-1 text-left">Concepto</th><th className="p-1">Factura</th><th className="p-1">Cálculo</th><th className="p-1">Diferencia</th></tr></thead><tbody>
        <tr><th className="py-1 text-left font-medium">Bruto</th><td className="p-1">{formatInvoiceValue(evidence.officialGrossPay)}</td><td className="p-1">{formatInvoiceValue(grossPay)}</td><td className="p-1">{formatInvoiceValue(evidence.grossVariance, true, true)}</td></tr>
        <tr><th className="py-1 text-left font-medium">Facturado</th><td className="p-1">{formatInvoiceValue(evidence.officialBilledAmount)}</td><td className="p-1">{formatInvoiceValue(billedAmount)}</td><td className="p-1">{formatInvoiceValue(evidence.billingVariance, true, true)}</td></tr>
      </tbody></table>
      <p>Diferencias de horas (RONOS − factura): regular {formatInvoiceValue(evidence.regularHoursVariance, false, true)} h; OT {formatInvoiceValue(evidence.overtimeHoursVariance, false, true)} h; PTO {formatInvoiceValue(evidence.ptoHoursVariance, false, true)} h.</p>
      <p>{evidence.identityVerified ? 'Tarifa histórica: factura oficial.' : 'Tarifa del candidato en factura; asignación pendiente de verificar.'} {evidence.officialName ? `Nombre en factura: ${evidence.officialName}. ` : ''}Tarifa factura: {formatInvoiceValue(evidence.officialPayRate, true)}/h; facturación: {formatInvoiceValue(evidence.officialBillRate, true)}/h.</p>
      <p className="break-all text-slate-500 dark:text-slate-400">Fuente: {evidence.sourceFile || 'Sin identificar'}</p>
    </div>
  </details>
}
...
function computeCingularBiWeeklyPeriods(weeks: WorkWeekOption[]): Array<{
  ...
  const ANCHOR_DATE = new Date('2026-08-10T00:00:00Z') // Calendar arithmetic must not shift at DST boundaries.
  ...
  for (const w of weeks) {
    ...
    const sDate = new Date(w.startDate.substring(0, 10) + 'T00:00:00Z')
...
function RonosLaborAuditContent() {
  ...
  const [payrollFilterType, setPayrollFilterType] = useState<'all' | 'paystub_match' | 'estimated' | 'variance' | 'pto' | 'violations'>('all')
  ...
  const handleStoreChange = async (newCompanyId: number) => {
    ++storeRequestRef.current
    ++payrollRequestRef.current
    payrollAbortRef.current?.abort()
    setPayrollData(null)
    setPayrollError(null)
    if (activeTab === 'payroll') setPayrollLoading(true)
  ...
  const fetchStoreAudit = async (
    ...
    try {
      ...
      if (Array.isArray(json.weeks) && json.weeks.length > 0) {
        ...
        throw new Error('No hay semanas disponibles para esta tienda. No se puede calcular el período.')
      }
      ...
      if (activeTab === 'payroll') {
        setPayrollError(err?.message || 'No se pudo resolver el período de nómina de la tienda.')
  ...
  useEffect(() => {
    ...
    // Cargar catálogos base; no sobrescribir una selección realizada mientras responde.
    const catalogRequestId = storeRequestRef.current
    fetch('/api/ronos/punches?companyId=34&_t=' + Date.now())
      .then(res => res.json())
      .then(json => {
        if (catalogRequestId !== storeRequestRef.current) return
        if (Array.isArray(json.stores)) setStores(json.stores)
        if (Array.isArray(json.weeks) && json.weeks.length > 0) {
          setWeeks(json.weeks)
          const computedPeriods = computeCingularBiWeeklyPeriods(json.weeks)
  ...
  const fetchPayroll = async (
    ...
    payrollAbortRef.current = controller
    ...
    try {
      ...
      if (!res.ok || !json?.success || !Array.isArray(json?.data?.employees)) {
        throw new Error(json?.error || `No se pudo obtener la nómina (${res.status}).`)
      }
      setPayrollData(json.data)
    } catch (err: any) {
      if (requestId !== payrollRequestRef.current || controller.signal.aborted) return
      console.error('Fetch payroll error:', err)
      setPayrollError(err?.message || 'Error al cargar la nómina. Vuelve a intentar.')
    } finally {
  ...
  const filteredPayrollEmployees = useMemo(() => {
    ...
    return payrollData.employees.filter((emp: any) => {
      ...
      const matchSearch =
        ...
        Boolean(emp.invoiceEvidence?.employeeNumber && String(emp.invoiceEvidence.employeeNumber).includes(query)) ||
        Boolean(emp.invoiceEvidence?.invoiceId && String(emp.invoiceEvidence.invoiceId).toLowerCase().includes(query)) ||
      ...
      if (payrollFilterType === 'paystub_match') {
        return emp.auditStatus === 'paystub_match'
      }
      if (payrollFilterType === 'estimated') {
        return emp.invoiceEvidence ? !emp.invoiceEvidence.identityVerified || emp.invoiceEvidence.status === 'incomplete' : !['paystub_match', 'variance'].includes(emp.auditStatus)
      }
      if (payrollFilterType === 'variance') {
        return emp.auditStatus === 'variance' || emp.invoiceEvidence?.status === 'variance' ||
          (emp.invoiceEvidence && [emp.invoiceEvidence.grossVariance, emp.invoiceEvidence.billingVariance, emp.invoiceEvidence.regularHoursVariance, emp.invoiceEvidence.overtimeHoursVariance, emp.invoiceEvidence.ptoHoursVariance].some(value => typeof value === 'number' && Number.isFinite(value) && Math.abs(value) >= 0.005))
      }
  ...
  const workspaceSections = [
    {
      id: 'store' as const,
      eyebrow: '1. Operación',
      title: 'Revisar mi tienda',
      description: 'Quién trabajó, quién necesita atención y qué tarjeta revisar.',
      icon: Building2,
      status: brokenCount > 0 ? `${brokenCount} por revisar` : 'Sin alertas críticas',
      tone: brokenCount > 0 ? 'rose' : 'emerald'
    },
    {
      id: 'chain' as const,
      eyebrow: '2. Dirección',
      title: 'Comparar tiendas',
      description: 'Detecta dónde concentrar la atención esta semana.',
      icon: BarChart3,
      status: `${chainData?.totalStores || 16} ubicaciones`,
      tone: 'blue'
    },
    {
      id: 'mapping' as const,
      eyebrow: '3. Datos',
      title: 'Confirmar personas',
      description: 'Comprueba que RONOS, Toast y Simplify hablen de la misma persona.',
      icon: LinkIcon,
      status: mappingStats.unmapped > 0 ? `${mappingStats.unmapped} pendientes` : 'Vínculos al día',
      tone: mappingStats.unmapped > 0 ? 'amber' : 'emerald'
    },
    {
      id: 'payroll' as const,
      eyebrow: '4. Nómina',
      title: 'Revisar proyección',
      description: 'Estima nómina y compara únicamente la evidencia disponible.',
      icon: Receipt,
      status: payrollLoading ? 'Calculando…' : payrollData ? 'Lista para revisar' : 'Elige un período',
      tone: payrollError ? 'rose' : 'violet'
    }
  ]
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans transition-colors duration-200">
        ...
        <div className="w-full px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-1 text-xs font-semibold tracking-wider uppercase select-none">
              <span className="hidden xl:inline text-white/90">Datos protegidos</span>
              <ShieldCheck className="w-3.5 h-3.5 text-white/90 ml-0.5" />
            </div>
          </div>
        </div>
      ...
      <main className="max-w-[1600px] mx-auto p-3 sm:p-5 lg:p-6 space-y-5">
        ...
        {/* Capa de orientación: el módulo siempre explica qué mirar y cuál es el siguiente paso. */}
        <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-sky-950 px-5 py-6 text-white sm:px-7">
            <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
              <div className="max-w-2xl">
                <p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-sky-300">Centro de control laboral</p>
                <h2 className="text-2xl font-black tracking-tight sm:text-3xl">¿Qué quieres resolver hoy?</h2>
                <p className="mt-2 text-sm leading-6 text-slate-300">RONOS reúne asistencia, cumplimiento y nómina. Empieza por una pregunta de negocio; el detalle aparece solo cuando lo necesitas.</p>
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                <div className="rounded-xl border border-white/10 bg-white/10 px-3 py-2"><span className="block text-slate-300">Ubicación</span><strong className="block truncate text-white">{activeStoreName}</strong></div>
                <div className="rounded-xl border border-white/10 bg-white/10 px-3 py-2"><span className="block text-slate-300">Período</span><strong className="block text-white">{formatUsaDateRange(storeData?.startDate, storeData?.endDate) || 'Selecciona una semana'}</strong></div>
                <div className="col-span-2 rounded-xl border border-white/10 bg-white/10 px-3 py-2 sm:col-span-1"><span className="block text-slate-300">Prioridad</span><strong className="block text-white">{brokenCount > 0 ? `${brokenCount} alertas` : 'Todo en orden'}</strong></div>
              </div>
            </div>
          </div>

          <div className="grid gap-3 p-4 lg:grid-cols-4 sm:p-5">
            {workspaceSections.map(section => {
              const Icon = section.icon
              const selected = activeTab === section.id
              const toneClass = section.tone === 'rose' ? 'bg-rose-50 text-rose-700 ring-rose-200 dark:bg-rose-950/30 dark:text-rose-300' : section.tone === 'amber' ? 'bg-amber-50 text-amber-800 ring-amber-200 dark:bg-amber-950/30 dark:text-amber-300' : section.tone === 'emerald' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-300' : section.tone === 'violet' ? 'bg-violet-50 text-violet-700 ring-violet-200 dark:bg-violet-950/30 dark:text-violet-300' : 'bg-sky-50 text-sky-700 ring-sky-200 dark:bg-sky-950/30 dark:text-sky-300'
              return <button key={section.id} onClick={() => { setActiveTab(section.id); setSelectedEmployeeDetail(null) }} className={`group rounded-2xl border p-4 text-left transition-all ${selected ? 'border-sky-500 bg-sky-50/70 shadow-sm ring-1 ring-sky-500 dark:border-sky-400 dark:bg-sky-950/30' : 'border-slate-200 hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-sm dark:border-slate-800 dark:hover:border-sky-700'}`}>
                <div className="flex items-start justify-between gap-3"><div className={`rounded-xl p-2.5 ${toneClass}`}><Icon className="h-5 w-5" /></div>{selected && <span className="rounded-full bg-sky-600 px-2 py-1 text-[10px] font-bold text-white">ABIERTO</span>}</div>
                <p className="mt-4 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{section.eyebrow}</p><h3 className="mt-1 text-base font-black text-slate-900 dark:text-white">{section.title}</h3><p className="mt-1 min-h-10 text-xs leading-5 text-slate-500 dark:text-slate-400">{section.description}</p><span className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${toneClass}`}>{section.status}</span>
              </button>
            })}
          </div>

          <div className="grid gap-3 border-t border-slate-100 bg-slate-50 px-4 py-4 sm:grid-cols-[1fr_1fr_auto] sm:px-5 dark:border-slate-800 dark:bg-slate-950/40">
            <label className="text-xs font-bold text-slate-600 dark:text-slate-300"><span className="mb-1.5 block">1. Elige ubicación</span><select value={selectedCompanyId} onChange={e => handleStoreChange(Number(e.target.value))} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-semibold text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-white"><option value={0}>Todas las tiendas</option>{stores.map(store => <option key={store.ronosCompanyId} value={store.ronosCompanyId}>{store.tegName}</option>)}</select></label>
            <label className="text-xs font-bold text-slate-600 dark:text-slate-300"><span className="mb-1.5 block">2. Elige semana</span><select value={selectedWeekId ?? ''} onChange={e => { const weekId = Number(e.target.value); setSelectedWeekId(weekId); if (!weekId) return; if (selectedCompanyId === 0) { fetchChainAudit(weekId) } else { fetchStoreAudit(selectedCompanyId, weekId) } }} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm font-semibold text-slate-900 dark:border-slate-700 dark:bg-slate-900 dark:text-white">{weeks.map(week => <option key={week.weekId} value={week.weekId}>{formatUsaDateRange(week.startDate, week.endDate)}</option>)}</select></label>
            <button onClick={handleSyncLive} disabled={syncing || loading} className="self-end rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-sky-500 dark:hover:bg-sky-400"><RefreshCw className={`mr-2 inline h-4 w-4 ${syncing ? 'animate-spin' : ''}`} />{syncing ? 'Actualizando…' : 'Actualizar datos'}</button>
          </div>
        </section>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs p-5 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Estimación de nómina y comparación del sueldo bruto con paystubs disponibles.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    <Download className="w-3.5 h-3.5" />
                    <span>Exportar estimación CSV</span>
                  </a>
                </div>
              </div>
              ...
              {/* ═══════════════════════════════════════════════════════════════════════════ */}
              {payrollError && <div role="alert" className="p-4 rounded-xl bg-rose-50 text-rose-800 border border-rose-200">{payrollError}</div>}
              {payrollLoading && <p role="status" className="text-sm text-slate-500">Cargando el período seleccionado…</p>}
              {payrollData && !payrollLoading && <>
              {payrollData.invoiceReconciliation && <PayrollInvoicePanel evidence={payrollData.invoiceReconciliation} payrollVerification={payrollData.invoicePayrollVerification} />}
              <section className="rounded-2xl border border-sky-200 bg-sky-50 p-4 text-slate-800 dark:border-sky-900 dark:bg-sky-950/30 dark:text-slate-100">
                <div className="flex gap-3">
                  <div className="mt-0.5 rounded-xl bg-sky-600 p-2 text-white"><Info className="h-4 w-4" /></div>
                  <div className="min-w-0">
                    <h3 className="font-black text-sm">Resumen de este período</h3>
                    <p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">Aquí solo revisas la nómina. Los pagos reales se realizan en Simplify HR.</p>
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      <div className={`rounded-xl border px-3 py-3 text-xs ${payrollData.unmatchedPaystubsCount > 0 ? 'border-amber-200 bg-white dark:border-amber-900 dark:bg-slate-900' : 'border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/30'}`}>
                        <strong className="block text-sm text-slate-900 dark:text-white">{payrollData.unmatchedPaystubsCount > 0 ? `${payrollData.unmatchedPaystubsCount} recibos pendientes de revisar` : 'No hay recibos pendientes'}</strong>
                        <p className="mt-1 leading-5 text-slate-600 dark:text-slate-300">{payrollData.unmatchedPaystubsCount > 0 ? 'No falta dinero ni se detiene ningún pago. El equipo administrativo puede revisarlos después.' : 'Todos los recibos disponibles quedaron identificados.'}</p>
                      </div>
                      <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-3 text-xs text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-100">
                        <strong className="block text-sm">{payrollData.salariedPaystubsWithoutRonosCount || 0} pagos administrativos ya reconocidos</strong>
                        <p className="mt-1 leading-5">Incluyen pagos de supervisores. Están correctos y no requieren reloj RONOS.</p>
                      </div>
                    </div>
                    {payrollData.coverage?.status === 'partial' && <p className="mt-3 rounded-xl bg-rose-50 px-3 py-2 text-xs font-bold text-rose-800 dark:bg-rose-950/30 dark:text-rose-200">Faltan tiendas o semanas para este período; el total es parcial.</p>}
                  </div>
                </div>
              </section>
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
                        {payrollInvoiceMode === 'consolidated' ? '🌟 Estimación consolidada' : payrollInvoiceMode === 'regular' ? '📄 Estimación ordinaria' : '⚡ Finiquitos separados'}
                      </span>
                    ...
                    <h3 className="text-sm font-semibold text-slate-300">
                      Facturación estimada del período
                    </h3>
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-xl">
                      {payrollData.invoiceReconciliation ? 'Proyección comparada con los documentos del panel de conciliación; revisa diferencias y cobertura.' : 'Proyección calculada con los datos disponibles; pendiente de conciliación con factura oficial.'}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full lg:w-auto shrink-0">
                    ...
                    <div className={`rounded-xl p-3 border min-w-[170px] ${payrollSupplementalsCount > 0 ? 'bg-amber-950/30 border-amber-500/40' : 'bg-slate-800/80 border-slate-700/60'}`}>
                      <div className="flex items-center justify-between text-[11px] font-semibold mb-1">
                        <span className={payrollSupplementalsCount > 0 ? 'text-amber-300' : 'text-slate-400'}>Suplementaria estimada</span>
                    ...
                    <div className="bg-slate-800/80 rounded-xl p-3 border border-slate-700/60 min-w-[170px]">
                      <div className="flex items-center justify-between text-[11px] text-slate-400 font-semibold mb-1">
                        ...
                        <span className="font-mono text-[10px] text-slate-500">Pendiente</span>
                      </div>
                      <div className="text-lg font-bold text-white">
                        Sin conciliar
                      </div>
                      ...
                      <div className="mt-1 space-y-0.5">
                        {payrollSupplementalsList.map((s: any, idx: number) => (
                          <div key={idx} className="text-xs text-amber-900 dark:text-amber-200 font-semibold">
                            • <span className="underline">{s.employeeFullName}</span> — Referencia <span className="font-mono">{s.invoiceCode || 'Sin identificar'}</span> estimada en <span className="font-bold">${(Number(s?.invoicedAmount) || 0).toFixed(2)}</span> (Cheque {s.checkNumber || 'sin identificar'})
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 text-center shadow-2xs">
                  ...
                  <span className="text-[10px] text-emerald-600 block mt-0.5">{payrollData.invoiceReconciliation ? 'Comparación documental disponible' : 'Pendiente de factura oficial'}</span>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 text-center shadow-2xs">
                  ...
                  <span className="text-[10px] text-blue-600 block mt-0.5">Sueldos brutos calculados</span>
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-center shadow-2xs">
                  ...
                  <span className="text-[10px] text-amber-600 block mt-0.5">Margen y cargos PEO estimados</span>
                </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  <button
                    onClick={() => setPayrollFilterType('paystub_match')}
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold cursor-pointer transition-colors ${
                      payrollFilterType === 'paystub_match'
                        ? 'bg-emerald-600 text-white'
                        : 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300 hover:bg-emerald-100'
                    }`}
                  >
                    Bruto coincide con paystub
                  </button>
                  <button
                    onClick={() => setPayrollFilterType('estimated')}
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold cursor-pointer transition-colors ${
                      payrollFilterType === 'estimated'
                ...
                <table className="w-full text-xs text-left">
                  <thead className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold border-b border-slate-200 dark:border-slate-700">
                    <tr>
                      ...
                      <th className="py-2.5 px-3 text-center font-bold text-emerald-700 dark:text-emerald-400">Factura estimada</th>
                    ...
                    {payrollLoading ? (
                      <tr>
                        <td colSpan={15} className="py-12 text-center text-slate-400">
                          <RefreshCw className="w-6 h-6 animate-spin mx-auto mb-2 text-[#03a9f4]" />
                          <span>Calculando estimación de nómina...</span>
                        </td>
                      </tr>
                      ...
                      filteredPayrollEmployees.map((emp: any, idx: number) => (
                        <tr key={`${emp.employeeUserId}-${emp.siteName || idx}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                          ...
                          <td className="py-2.5 px-3 text-center">
                            <span
                              title={emp.auditNote || 'Pendiente de conciliación con paystub y factura oficial'}
                              className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold cursor-help ${
                                emp.auditStatus === 'paystub_match'
                                  ? 'bg-emerald-100 text-emerald-800'
                            ...
                            >
                              {emp.auditStatus === 'paystub_match' ? 'Bruto coincide con paystub' : emp.auditStatus === 'variance' ? 'Diferencia de bruto' : 'Estimado / sin conciliar'}
                            </span>
                            {typeof emp.officialGrossPay === 'number' && Number.isFinite(emp.officialGrossPay) && <div className="text-[10px] mt-1">Paystub: ${emp.officialGrossPay.toFixed(2)}</div>}
                            {typeof emp.grossPayVariance === 'number' && Number.isFinite(emp.grossPayVariance) && <div className="text-[10px]">Diferencia: ${emp.grossPayVariance.toFixed(2)}</div>}
                            {emp.invoiceEvidence && <PayrollInvoiceEmployeeDetail evidence={emp.invoiceEvidence} grossPay={emp.totalGrossPay} billedAmount={emp.totalInvoicedAmount} />}
                          </td>
                        </tr>
                      ))
                    )}
